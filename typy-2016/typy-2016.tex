...
\title{Dynamically Composable Type System Fragments}
...
Programmers often develop new languages when they want to express new semantic structures. The problem with this \emph{language-oriented} approach is that sibling languages cannot always cleanly interact. This paper instead pursues a more compositional \emph{fragment-oriented} approach to the problem of expressing new semantic structures. 
In particular, we define a semantics that delegates control over each term drawn from a fixed syntax to some contextually relevant user-defined \emph{fragment definition}. 
...
The delegated fragment is tasked with typechecking the term and  with assigning dynamic meaning to the term by computing its translation to a fixed internal language. Syntactic conflicts cannot occur and the meaning of a program remains stable under fragment composition (unlike systems where the semantics is a ``bag of rules''.) 

We formalize this mechanism with a bidirectionally typed translation semantics. We also introduce $\typy$, a practical implementation of this mechanism embedded into Python by syntactic reflection. We establish the expressive power of this mechanism by showing fragments that define 1) functional record types; 2) a variant of JavaScript's prototypic object system; 3) labeled sum types (with support for nested pattern matching \emph{a la} ML); and 4) foreign function interfaces with Python and OpenCL.% We formalize this mechanism as a small bidirectionally typed lambda calculus and prove that 
...
These designs address the problem of \emph{backwards compat\-ibility}, but as languages like these continue to proliferate (aided by language workbenches and other tools \cite{erdweg2013state}), language designers face the more difficult problem of \emph{lateral compatibility}, i.e. of interfacing with libraries  written in sibling languages. 
...
% In this paper, we seek to redeem the \emph{library-oriented approach}. 
% in favor of the \emph{language-oriented approach} \cite{journals/stp/Ward94} is, in many cases,  premature. 
In this paper, we pursue a \emph{fragment-oriented approach} to the problem of semantic evolution. Rather than packaging, for example, functional sums and products or novel object systems into monolithic language definitions, we introduce a language mechanism that allows programmers to express semantic structures like these as composable \emph{fragments}, which are distributed in libraries. This sidesteps the lateral compatibility problem -- if a library requires clients to manipulate, e.g., functional records, it can simply designate the library that defines the record fragment as a dependency. 
...
Of course, designing a  mechanism capable of compositionally expressing what are usually understood as the semantic primitives of a language presents several  challenges.
...
First, language designers typically define new concrete forms for the primitive structures that they introduce, but if we allow fragment providers to define new concrete forms in a decentralized manner (following Sugar* \cite{erdweg2013framework} and related systems), then different fragments could define conflicting forms. For example, consider the following forms:
...
At the level of abstract syntax, we encounter the classic \emph{expression problem} \cite{wadler1998expression,Reynolds75}: if fragment providers can define new term constructors, then it becomes difficult to define functions that proceed by exhaustive case analysis over the  constructors, like pretty-printers.
...
We address the problems of syntax very simply: fragment providers cannot extend the concrete nor the abstract syntax of our language.  Instead, the semantics allows fragments to ``share'' syntactic forms by delegating control over each term to a contextually relevant fragment definition. This choice is deterministic and stable under fragment composition, so conflicts do not arise. 
The delegated fragment is tasked first with typechecking the term. If typechecking fails, the fragment generates the error message. If typechecking succeeds, the fragment is then tasked with assigning dynamic meaning to the term by computing a translation to a fixed \emph{internal language} (IL). The IL is analagous to an intermediate language as one might find in the first stage of a compiler (here shifted into the semantics of the language.)
