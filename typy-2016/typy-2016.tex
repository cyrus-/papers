...
If typechecking is successful, $\typy$ delegates to the same fragment to generate a translation. For example, $\typy$ calls the \li{record.trans_Dict} method shown in Figure \ref{fig:record-intro}, which translates records to Python tuples (the labels do not need to appear in the translation.) This method asks $\typy$ to determine translations for the field values by calling \li{ctx.trans}. The field ordering is alphabetical.
