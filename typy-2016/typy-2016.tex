...
with pat: e
\end{lstlisting}
where \li{pat} is a pattern and \li{e} is the corresponding \emph{branch expression}. $\typy$ also supports an expression-level match construct, discussed in Appendix \ref{appendix:match-expressions}.
...
It would also be interesting to automate the generation of fragment definitions from inductive specifications, building on the techniques developed by the Veritas project \cite{grewe2015type}.
