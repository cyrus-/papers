...
single ``extensible'' language, $\typy$, that allows library providers to package a variety of semantic structures into  \emph{semantic fragments} that library clients can import in any combination. %an ``extensible'' language that  
...
We introduce various aspects of $\typy$'s fragment system with simple examples in Sec. \ref{sec:typy-by-example}, then describe more sophisticated case studies in Sec. \ref{sec:more-examples}. %Programmers do not need to adopt new tools to use \texttt{typy}. 
...
We summarize related work in Sec. \ref{sec:related-work}  and conclude after a discussion of present limitations and future work in Sec. \ref{sec:discussion}.
...
If typechecking is successful, $\typy$ delegates to the same fragment to generate a translation. For example, $\typy$ calls the \li{record.trans_Dict} method shown in Figure \ref{fig:record-intro}, which translates records to Python tuples (the labels do not need to appear in the translation.) This method asks $\typy$ to determine translations for the field values by calling \li{ctx.trans}.
...
Proof assistants, e.g. TinkerType \cite{LevinPierce99}, PLT Redex \cite{Felleisen-Findler-Flatt09}, Agda \cite{norell2007towards} and Coq \cite{Coq:manual} can be used to specify and mechanize the metatheory of languages, but these require a complete specification (this  has been identified as a key challenge \cite{aydemir05tphols}.) Techniques for composing specifications and proofs exist \cite{conf/popl/DelawareOS13,Delaware11,conf/plpv/SchwaabS13}, relying on various algebraic methods to encode ``open'' term encodings (e.g. Mendler-style $f$-algebras \cite{conf/popl/DelawareOS13}) but they  require additional proofs at ``combine-time'' and provide no guarantees that \emph{all} fragments having some modularly established property can be used together. %For example, Delaware et al. develop a technique based on software product lines that requires specifying assumptions at feature boundaries and providing proofs of these for each composition \cite{Delaware11}. In later work based on Mendler-style $f$-algebras, this problem of \emph{feature interactions} persists for many lemmas, including those related to tycon invariants (e.g. canonical forms) \cite{conf/popl/DelawareOS13}. 
Several authors, notably Chlipala  \cite{Chlipala10}, suggest  proof automation as a heuristic solution to the problem of combine-time proof obligations. Our approach is more pragmatic and ``implementation focused'', in that no inductive specification of the intended semantics needs to be given. 

Refinement types / pluggable type systems 

Object algebras?

LMS \cite{Rompf:2012:LMS} supports staged translation of well-typed Scala programs to other targets and supports composable optimizations. Macro systems can also be used to define optimizations and operations with interesting dynamic semantics. In these systems, the language's type system itself remains fixed. Our fragment system permits true type system extensions. New types are not merely aliases, nor must their rules be admissible in some base type system (e.g. \lip{string_in} would be difficult to define in Scala, particularly as directly as we can in $\typy$.) % Note that in \texttt{typy}, macros can be seen as a mode of use of the term constructor \verb|Call|, as we can give the macro a singleton type (like Python modules, discussed above) that performs the desired rewriting in \verb|ana_Call| and \verb|syn_Call|.


{\it Operator overloading} \cite{vanWijngaarden:Mailloux:Peck:Koster:Sintzoff:Lindsey:Meertens:Fisker:acta:1975} and {\it metaobject dispatch} \cite{Kiczales91} are protocols that translate operator invocations into function calls. The function is typically selected according to either the type or the dynamic tag or value of one or more operands. These protocols share the notion of {\it inversion of control} with our strategy for targeted expressions, described in Sec. \ref{sec:targeted-forms}. However, our strategy is a {\it compile-time} protocol and gives control over typing and translation. An object system with support for operator overloading could be implemented using our fragment system. %Note that we used Python's operator overloading and metaobject protocol for convenience in the static language. %Our dispatch protocol was designed to be conceptually similar to Python's operator overloading protocol \cite{python}.
...
Because $\typy$ fragments are implemented in Python, it is difficult to rigorously prove metatheoretic properties about them (though formal specifications of a large portion of Python do exist \cite{politz2013python}.) In future work, we plan to design a fragment system where the fragment definition language is dependently typed, which would make it possible to prove interesting properties about fragments and, ideally, combine these proofs automatically. A complementary approach would be to automate the generation of fragment definitions and accompanying proofs from declarative specifications, following the approach of the Veritas system \cite{grewe2015type}.
...
We are actively developing the \verb|typy| library and its surrounding ecosystem as a free open source project. The \lip{typy} website will be found at \url{http://www.cs.cmu.edu/~comar/typy/}.
