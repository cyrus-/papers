...
\newcommand{\lip}[1]{\lstinline[language=Python,basicstyle=\ttfamily,deletendkeywords={tuple,buffer,map}]{#1}}
...
This paper introduces $\typy$, a bidirectionally typed language embedded into Python by syntactic reflection. Rather than defining a monolithic semantics, $\typy$  delegates semantic control over each term drawn from Python's fixed syntax to some implicitly relevant \emph{semantic fragment}. 
...
This fragment programmatically 1)  typechecks the term, and then 2) gives dynamic meaning to the term by computing its translation to Python. 
This design is theoretically well-behaved: it side\-steps the expression problem and the problem of  syntactic conflict, and programs are semantically stable under fragment composition (unlike systems where the semantics is a ``bag of rules.'') 
To demonstrate the expressive power of this mechanism, we discuss user-defined fragments that define 1) functional record types; 2) a variation on JavaScript's prototypic object system; 3) labeled sum types (with support for nested pattern matching \emph{a la} ML); and 4)   foreign function interfaces to Python and OpenCL. 
...
In this paper, we pursue an alternative \emph{library-oriented approach} to the problem of packaging semantic fragments. %: rather than packaging, for example, new  object systems or variations on functional products and sums into different ``monolithic'' languages, we seek to express them as composable \emph{semantic fragments} packaged into libraries for a single ``extensible'' language. %an ``extensible'' language that  
...
In Sec. \ref{sec:typy-by-example}, we describe  \texttt{typy}, which repurposes  Python's syntax and uses Python itself as the IL \cite{sanner1999python,python}. There are two main reasons for this choice: 1) Python supports both syntactic reflection and dynamic code generation, so we can bootstrap this mechanism itself as a Python library; and 2) Python's syntax, cleverly repurposed, is rich enough to express a wide variety of interesting constructs cleanly. We detail simple examples in Sec. \ref{sec:typy-by-example} and then describe several other more sophisticated examples in Sec. \ref{sec:more-examples}, including...\todo{list examples (?)}%Programmers do not need to adopt new tools to use \texttt{typy}.
...
\begin{codelisting}[t]
\begin{lstlisting}
from typy.std import (component, record, string,  ^\label{fig:record-example/line:import-start}^
  string_in, dyn) ^\label{fig:record-example/line:import-end}^
...
def Listing1():
  ...
  test_acct [: Account] = {
    ...
    memo:        None
  }^\label{fig:record-example/line:component-end}^
\end{lstlisting}
\caption{Types and values in $\typy $.}
\label{fig:record-example}
\end{codelisting}

Listing \ref{fig:record-example} shows an example of a well-typed \texttt{typy} program. %We will refer to it throughout this section.

\paragraph{Dynamic Embedding} \texttt{typy} is dynamically embedded into Python, so Listing \ref{fig:record-example} is simply a standard Python script at the top level. $\typy$ supports versions 2.6+ and 3.0+ of Python, though in later examples, we will use syntactic conveniences available only under versions 3.0+. Appendix \ref{sec:python2-support} discusses the changes necessary to port these examples to Python 2.6+.% The execution of this script begins the \emph{static phase} of compilation, as suggested by line \ref{hello-ct}. The main purpose of this phase is to define the tycons and types that will be used by the \emph{typed functions} in the program, marked in this example by the decorator \lip{@fn}. The static phase ends once  these typed functions have gone through the process of active typechecking and translation. The generated translation is what ultimately determines the run-time behavior of the program. % The evaluation of the translation of a typed function represents the \emph{dynamic phase}. %For example, the string on line \ref{hello-ct} is a statically valued string. Types are also statically valued, as we will discuss shortly. 
...
\paragraph{Package Management} Embedding $\typy$ into Python pays off immediately on the first line of Listing \ref{fig:record-example}: we can use Python's import mechanism directly, so Python's package management tools (e.g. \lip{pip}) and package repostories (e.g. \lip{PyPI}) are directly available for distributing \texttt{typy} libraries, including those defining semantic fragments. 

All imports on Lines \ref{fig:record-example/line:import-start}-\ref{fig:record-example/line:import-end} are fragments that are defined in the \verb|typy| standard library, \texttt{typy.std}. The semantics of $\typy$ does not fundamentally privilege the standard library.% fragments are simply defined in the same library and distributed with the language for convenience.
...
\paragraph{Top-Level Values}
On Lines \ref{fig:record-example/line:component-start}-\ref{fig:record-example/line:component-end}, we define a top-level $\typy$ value named \lip{Listing1} by decorating a Python function definition with a fragment, \lip{component}. Decorator syntax in Python is syntactic sugar for applying the decorator to the function value being decorated as a transformation. Here, the decorator is a $\typy$ fragment, not a Python function, but $\typy$ uses Python's operator overloading to allow fragments to function as decorators at the top level. The decorated Python function is discarded entirely after $\typy$ extracts its abstract syntax tree and \emph{static environment}, i.e. its closure and the globals dictionary of the surrounding Python module, using the reflection capabilities exposed by the \lip{inspect} and \lip{ast} packages in Python's standard library.\footnote{The reader may need to refer to the  documentation for the \lip{ast} package, available at \url{https://docs.python.org/3.6/library/ast.html}, to fully understand the examples in the remainder of this section.}

\paragraph{Statements}
Next, $\typy$ delegates control over the statements in the body of the definition to the fragment. In this example, there are two assignment statements.
