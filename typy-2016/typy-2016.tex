...
We argue that this design is \emph{expressive} with examples of fragments that express the semantics of 1) functional record types; 2) a variation on JavaScript's prototypic object system; 3) labeled sum types (with nested pattern matching \emph{a la} ML); and 4)   foreign interfaces to Python and OpenCL. 
...
As new programming languages continue to proliferate, programmers face the daunting problem of \emph{lateral compatibility}, i.e. of interfacing with libraries written in sibling languages. For example, there are useful libraries written in TypeScript \cite{bierman2014understanding}, Flow \cite{Flow} and PureScript \cite{PureScript}, but these libraries are not directly accessible across language boundaries because these languages are syntactically and semantically incompatible (the first two define different object systems, and PureScript is a functional language similar to Haskell and ML.) 

One common workaround is to interface indirectly with these libraries through the code generated by a compiler that targets a more established language for which a \emph{foreign interface (FI)} is available. For example, all of the languages above have compilers that target JavaScript and they also all interface with JavaScript. Unfortunately, this approach is unnatural (the syntactic and semantic conveniences of the sibling language are unavailable), unsafe (the type system of the sibling language is not enforced) and anti-modular (if the compiler representations change, client programs can break.) It appears, then, that the language-oriented approach \cite{journals/stp/Ward94} is difficult to reconcile with the best practices of ``programming in the large'' \cite{DeRemer76}. 
...
In this paper, we argue for a more compositional \emph{fragment-oriented} approach to the problem of expressing new semantic structures. Rather than packaging, for example, new object systems or various functional  structures into ``monolithic'' languages, we seek to develop a single ``extensible'' language that allows library providers to package semantic structures like these into  \emph{semantic fragments} that library clients can import in any combination. %an ``extensible'' language that  
...
For example, clients of a library that uses functional records at its public interface can simply import the record fragment themselves (i.e. the library can list the record fragment as a dependency, in the usual manner.) % This approach sidesteps the lateral compatibility problem.

Of course, designing an expressive {semantic fragment system} equipped with useful composition principles presents several well-known challenges.
...
If typechecking is successful, $\typy$ next generates a translation by delegating to the same fragment. For example, $\typy$ calls the \li{record.trans_Dict} method shown in Figure \ref{fig:record-intro}, which translates records to Python tuples (the labels do not need to appear in the translation.) This method asks $\typy$ to determine the translations of the field values, following the same protocol,  by calling \li{ctx.trans}.
...
Like $\typy$, Typed Racket is a statically typed language embedded into a dynamically typed language \cite{TypedScheme2008}. However, the type system of Typed Racket is monolithic -- adding new semantic structures requires directly modifying the language.

In our recent work on \emph{type-specific languages (TSLs)} in the Wyvern language, we used bidirectional typechecking to control aspects of parsing literal syntax in a manner quite similar to, but somewhat more syntactically flexible than, how we treat literal forms in $\typy$ \cite{TSLs,sac15}. The increased flexibility comes because we had complete control over its grammar. By repurposing Python's syntax, we benefit from an established ecosystem of tools (e.g. parsers, syntax highlighters, editor modes, etc.) Unlike $\typy$, Wyvern has a monolithic semantics. % The Wyvern formalism guarantees hygiene, using an approach that is also likely applicable in the setting of this paper.

Qi (now Shen) is a typed Lisp dialect where library providers can introduce new typing rules declaratively \cite{tarver2008functional}. The type system is, in effect, a ``bag of inference rules.'' The problem with this approach is that it permits non-determinism (i.e. more than one rule may apply to a term.) The fragment delegation protocol of $\typy$, on the other hand, is deterministic and stable under fragment composition. This also has cognitive benefits, in that it is easy to determine which fragment's documentation to consult to understand a term.

Language-external mechanisms for creating and combining language dialects, e.g. extensible compilers like Xoc \cite{conf/asplos/CoxBCKK08} and various language workbenches \cite{erdweg2013state}, suffer from the same problem-- the combined dialect may be semantically ambiguous or alter the semantics of existing code. Most of these systems also permit syntax extension, and so also suffer from the problem of syntactic ambiguity. The Copper tool is a notable exception in that it offers a modular analysis that guarantees syntactic determinism, but this requires the use of of verbose marking tokens and grammar names \cite{conf/pldi/SchwerdfegerW09}. In contrast, $\typy$ allows many different fragments to share common syntactic forms without qualification.

Grewe's recent work

TinkerType and Ott ; Twelf ; Delaware's work

Proof assistants can be used to specify and mechanize the metatheory of languages, but also usually require a complete specification (this  has been identified as a key challenge \cite{aydemir05tphols}). Techniques for composing specifications and proofs exist \cite{conf/popl/DelawareOS13,Delaware11,conf/plpv/SchwaabS13}, but they  require additional proofs at composition-time and provide no guarantees that \emph{all} fragments having some modularly checkable property can be used together, as in our work. %For example, Delaware et al. develop a technique based on software product lines that requires specifying assumptions at feature boundaries and providing proofs of these for each composition \cite{Delaware11}. In later work based on Mendler-style $f$-algebras, this problem of \emph{feature interactions} persists for many lemmas, including those related to tycon invariants (e.g. canonical forms) \cite{conf/popl/DelawareOS13}. 
Several authors, notably Chlipala  \cite{Chlipala10}, suggest  proof automation as a heuristic solution to the problem. 


Object algebras

Refinement types / pluggable type systems 



Macros

LMS \cite{Rompf:2012:LMS} allows staged translation of well-typed Scala programs to other targets and supports composing optimizations. Macro systems can also be used to define optimizations and operations with interesting dynamic semantics. In both cases, the language's type system itself remains fixed. Our mechanism permits true type system extensions. New types are not merely aliases, nor must their rules be admissible in some base type system (e.g. \lip{string_in} would be difficult to define in Scala, particularly as directly as we can here). Note that in \texttt{typy}, macros can be seen as a mode of use of the term constructor \verb|Call|, as we can give the macro a singleton type (like Python modules, discussed above) that performs the desired rewriting in \verb|ana_Call| and \verb|syn_Call|.
