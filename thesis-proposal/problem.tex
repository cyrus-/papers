...
In the dissertation, I plan to first describe the simplest style of TSMs (i.e. unparameterized expression TSMs) by example. This is \textbf{complete}. I will next present a formal system that formalizes such TSMs. The account in Sec. \ref{sec:tsm-formalism} represents the first and most substantial step of this plan. This is complete on paper, so I mainly need to typeset the omitted details, including the metatheory. I have made substantial progress as of this writing (see link below) and I plan to finish this by the \textbf{middle of November}. This system will serve as the basis for several extensions that capture other constructs described above, so I will ask the committee to read this section in detail once it is complete.

The first extension will add support for pattern matching. I will base this extension on the account of pattern matching given in \emph{PFPL} \cite{pfpl}. I plan to have a rough sketch of how this should work by \textbf{late-November}, in consultation with the committee, and a final write-up by Black Friday (i.e. the \textbf{third week of December}). 

The next extension will add support for value, type and module parameters. To do so, I will assume that the available module paths and their signatures are being tracked by a module context in the static semantics, without detailing how the module language populates this context (i.e. I will only specify the ``core language''). I plan to sketch this out and discuss it with my committee \textbf{during November and December} and finish writing the details up by the \textbf{beginning of Spring semester}.

Beginning in the \textbf{Spring semester}, I will write up the work on type-specific languages. A variant of this work has been published already, and the groundwork for it will have already been laid down in the earlier chapters, so I anticipate being done with this by \textbf{the second week of February}. I will finish the remaining sections (e.g. additional background, related and future work sections, and additional examples), flesh out the proofs of minor lemmas and respond to the committee's requests for edits for the remainder of the semester. Based on this timeline, I plan to defend before the \textbf{end of the Spring 2016 semester}.
