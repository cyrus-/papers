...
Indeed, tools that assist with the construction of so-called  ``domain-specific'' language dialects are only becoming increasingly popular.\footnote{We consider only so-called ``external DSLs'' here. ``Internal DSL'' refers to a programming interface that  ``resembles'' that of a distinct dialect, though it is actually defined as a library \cite{fowler2010domain}.} 
