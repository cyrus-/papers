...
We will consider another example -- regular expression patterns -- in Sec. \ref{sec:examples}. 
...
\inferrule[TSM-OK]{
	\Delta \vdash \tau~\mathtt{type}\\
	\emptyset~\emptyset \vdash \hat{e}_\text{parse} \leadsto e_\text{parse} : \mathsf{Body} \rightharpoonup \mathsf{ExpOption}
}{
	\Delta \vdash \mathsf{syntax}~@~\tau~\{\hat{e}_\text{parse}\}~\mathtt{tsm}
}
\end{mathpar}
