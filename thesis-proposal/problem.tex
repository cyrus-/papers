...
\end{lstlisting}
Notice that the translations of the component regular string types are not inserted directly, but are rather treated \emph{parametrically} using the \emph{translational form} \lstinline{SCSStrans(ECSSTSCSS)}. This will be key to the modularity principle that we plan to explore in the remaining work.
...
\paragraph{Operator Constructors} On line \ref{line:intro-unlabeled-sig}, the metasignature \lstinline{LTUPLEX} specifies an operator constructor \lstinline{intro_unlabeled}. No parameter kind is listed, so the trivial parameter kind, \lstinline{unit}, is assumed, as indicated by the comment.
