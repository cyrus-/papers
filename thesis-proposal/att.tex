...
Typed programming languages can be considered in terms of the collection of indexed type and operator constructors they provide. The simply-typed lambda calculus (STLC), for example, provides a single type constructor, \fvar{Arrow}, indexed by a pair of types, and two operator constructors: $\opvar{lam}$, indexed by a type, and $\opvar{ap}$, which can be thought of as being indexed trivially. It's static semantics can be written abstractly like this, to emphasize this uniform way of thinking about type structure:
\begin{figure}[h]
\small
\vspace{-8pt}
\begin{mathpar}
...
\inferrule[arrow-I]{
	...
	\jetX{\opvar{lam}[\tau]({x}.e)}{\fvar{Arrow}[(\tau, \tau')]}
}
...
We might extend this language with universally quantified and recursive types by adding the type constructors $\fvar{Forall}$ and $\fvar{Mu}$, which are indexed by types under a binder, and their associated operator constructors (see \cite{pfpl} for a review). It is tempting to stop here: the language now supports general recursion (i.e. it is  Turing complete) and the dynamic semantics of sums, products and inductive and co-inductive types can be defined by a method analagous to Church encodings in the untyped lambda calculus \cite{reynolds}\todo{what to cite for this?}. This is of clear theoretical and pedagogical interest, but this calculus is plainly impractical as a programming language. Reynolds, striking a cautionary tone reminiscent of Perlis in his description of the ``Turing tarpit'' \cite{Perl82a}, put it succinctly \cite{Reynolds94anintroduction}: 
\begin{quote}
...
This issue was first discussed by Reynolds \cite{Reynolds75} and was dubbed the \emph{expression problem} by Wadler \cite{wadler1998expression}. A number of mechanisms have since been proposed that allow new cases to be added to data types and the functions that operate over them in a modular manner. 
...
If we allowed users to load definitions like these into our compiler, we will have succeeded in creating an  extensible compiler, albeit one where orthogonality is not guaranteed (we will return to this point). We have not, however, created an extensible programming language because other compilers for the same language will not necessarily support the same extensions. These mechanisms are not \emph{language-integrated}, so if our newly-introduced constructs are exposed at a library's  interface boundary, clients of that library who are using different compilers face the same problems with client compatibility that those using different languages face (as described in Sec. \ref{external-approaches}). %That is, {extending a language by extending a single compiler for it is morally equivalent to creating a new language}. 
...
\textbf{internal terms} 				& 	\iota	&	::=	&	\evar{x} \pipe 
												...
												\isnd{\iota}
												\\
							& & \pipe & \iinl{\sigma_2}{e} \pipe \iinr{\sigma_1}{e} \pipe \icase{e}{x}{e_1}{x}{e_2} \\
							& 		& 	\pipe	& 
...
\textbf{internal types}			&	\sigma	&	::=	&    \darrow{\sigma_1}{\sigma_2} \pipe \dpair{\sigma_1}{\sigma_2} \pipe \dsum{\sigma_1}{\sigma_2} \pipe \dint 
...
% 												\trepof{\tau} \pipe \dup{\tau}\\
	& \bar{\iota} & ::= & x \pipe \ifix{x}{\bar \sigma}{\bar \iota} \pipe \ilam{x}{\bar \sigma}{\bar \iota} \pipe \iapp{\bar \iota_1}{\bar \iota_2} \pipe \cdots \pipe \iup{\tau} \pipe \tvalof{\tauiterm}{\tautype} \\						
...
\end{array}$$
\vspace{-10pt}
\caption{\small Syntax of Core \atlam. Here, $x$ ranges over external and internal language variables, $\tvar{t}$ ranges over type-level variables, $\fvar{Tycon}$ ranges over type constructor names, $\opvar{op}$ ranges over operator constructor names, $\tstr{str}$ ranges over string literals, $\iintlit$ ranges over integer literals and $\oplus$ ranges over the standard total binary operations on integers.%The productions related to the internal language are written using generators $\mathcal{G}$ and $\mathcal{S}$ to avoid duplicating the syntax of common terms.
\label{grammar}}
\end{figure}
\begin{figure}[t]
\begin{lstlisting}
...
\label{nat-atlam}
\end{figure}
In this section, we will develop an ``actively typed'' version of the simply-typed lambda calculus with simply-kinded type-level computation called @$\lambda$. More specifically, the level of types, $\tau$, will itself form a simply-typed lambda calculus, where the ``types'' classifying type-level terms are called \emph{kinds} to distinguish them from the types classifying expressions, which are just one kind (written $\star$) of type-level value. In the semantics for this calculus, we will integrate typed compilation techniques and a form of type abstraction to allow us to prove key type safety, decidability and conservativity theorems. The syntax of @$\lambda$ is given in Fig. \ref{grammar}. An example of a program that introduces and uses primitive natural numbers, essentially implementing the semantics of G\"odel's \textbf{T} by translation to integers internally, is given in Fig. \ref{nat-atlam}\todo{use proper syntax}. We do not, of course, deny that natural numbers can be strongly encoded by existing means (e.g. as an abstract type); we will provide more sophisticated examples after explaining the core mechanism with this simple example. %It consists of:
...
A program, $\rho$, consists of a series of static declarations followed by an external term, $e$. The syntax for external term contains variables, fixpoints\todo{maybe we should actually call this @PCF???}, functions, and a uniform form for other operator invocations, which we will discuss below. Compiling a program consists of first \emph{kind checking} its static declarations and type-level terms, $\tau$, then \emph{type checking} the external term and, simultaneously, \emph{translating} it to a term, $\iota$, in the typed internal language. In our calculus, the internal language is essentially Plotkin's PCF with binary product and sum types and a single base type of integers. The typing judgement thus simultaneously relates an external term to a type and an internal term: 
$$\ecompilesX{e}{\tau}{\iota}$$ 

This judgement follows conventions for a typed lambda calculus in most respects. $\Gamma$ is a context mapping variables to types and obeys standard structural properties. The key novelty here is that the type and operator constructors available for use within $e$ are not fixed by the language, but rather are determined by the \emph{constructor context}, $\fvalCtx$, which can be extended by user-defined declarations.

\subsection{Type Constructors, Types and Type Equivalence}
New type constructors are declared at the top-level of a program using \textsf{tycon}. Each type constructor  must have a unique name, e.g. \fvar{Nat}. We do not consider the issue of naming conflicts, under the assumption that unique names will be generated by some external mechanism (e.g. a URI-based scheme, as in Java). Type constructors are indexed by type-level values, so the type constructor must also specify an \emph{index kind}, $\kappaidx$. To permit the implementation of interesting type systems, the type-level language includes additional kinds of data beyond types themselves. We lift several  standard functional types to the level of kinds: here, unit ($\kunit$), numbers ($\dint$), binary sums\todo{add sums to TL}, binary products ($\kpair{\kappa_1}{\kappa_2}$) and lists ($\klist{\kappa}$). We also includes labels, \todo{label kind}, which can be thought of as type-level strings whose bodies are restricted to prevent ambiguities in the concrete syntax we will later discuss. Labels have no elimination form (they can only be compared, in our calculus only for equality). Our first example, to avoid complexity, is indexed trivially, i.e. by unit kind, $\kunit$, but we will show examples of type constructors that are indexed in more interesting ways in later portions of this work. For example, $\fvar{Tuple}$ will be indexed by a list of types, and $\fvar{LabeledTuple}$ will indexed by a list pairing labels with types. Types are type-level values of kind $\star$ and are introduced by applying a declared type constructor to a type-level term of its index kind, e.g. $\ttype{Nat}{\tunit}$. Types can also be case analyzed to extract their index. Like open datatypes, there is no notion of exhaustiveness; there must always be a default case, here written \textsf{ow} (``otherwise'').

Type constructors are not first-class; they do not themselves have arrow kind as in some kind systems (e.g.  \cite{Watkins08}; Ch. 22 of \emph{PFPL} describes a related system \cite{pfpl}). This neatly avoids some complexities in these systems relating to the decidability of type equivalence. In our setting, two types are considered equivalent if they apply the same constructor, identified by name, to equivalent indices. Going further, we ensure that deciding type equivalence requires only checking for syntactic equality of normal forms (i.e. $\beta$-equivalence) by imposing the restriction that type constructors can only be indexed by kinds for which semantic equivalence coincides with  $\beta$-equivalence. This is acceptable for many interesting examples. We leave safely loosening these restrictions by introducing a notion of definitional equivalence of types and allowing extension providers to extend it as future work (this would impose non-trivial proof obligations onto extension developers that could not be resolved by a simply-kinded statics nor checked during normalization).

The type-level language does, however, include total type-level functions, which have arrow kind, $\karrow{\kappa_1}{\kappa_2}$. Type constructor application can be wrapped in a type-level function to emulate first-class type constructors (and indeed, such a wrapper could be generated automatically, though we avoid this for simplicity in our semantics). Equivalence at arrow kind does not coincide with $\beta$-equivalence, so type-level functions cannot appear in type indices. Our treatment of equivalence in the type-level language is thus quite similar to the treatment of term-level equality using ``equality types'' in a language like Standard ML. Indeed, one could justifiably suggest that in a practical implementation of this simply-kinded mechanism, the type-level language would most sensibly be based on a pure, total subset of ML (consistent with its early development as a ``MetaLanguage'' and the role of the type-level language as, in effect, a DSL for writing type systems and elaborators, which functional languages are widely considered well-suited for). We leave the development of such an implementation (and of bootstrapping a type-level language that is itself extensible) as promising future work.

\emph{representation schema} $\taurep$ 

\subsection{Operator Constructors and Elaborations}
