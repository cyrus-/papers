...
\begin{figure}[h]
\small
\hspace{-5px}\input{../att-popl15/syntax-table-IL}
...
We then continue to @\texttt{lang} in Sec. \ref{ace}. In @\texttt{lang}, the static language is Python and type constructors, which in the calculus were  tracked by a context and operated as a distinguished ``open'' datatype, can be implemented as subclasses of a built-in class, \verb|atlang.Type|, leveraging a simple form of inheritance for the same purpose. We support Python's syntax in a natural manner and have implemented a number of full-scale examples of statically-typed language extensions, extending the initially impoverished type system of Python substantially.% which we will briefly introduce here.
