...
In this paper, we begin from first principles by specifying a language, @$\lambda$, in a style that follows many full-scale languages: as a bidirectionally typed translation semantics. Only the $\rightarrow$ type constructor (tycon) is built in; all other external tycons (we show constrained strings and a variation on record types) are defined by extending a \emph{tycon context}. Each tycon defines the semantics of its associated term-level \emph{opcons} (e.g. row projection) using a static language where types and translations are values. The semantics guarantees \emph{type safety} and, critically, \emph{conservativity}: that all \emph{tycon-specific invariants} that can be established in a ``closed world'' will necessarily be conserved in the ``open world''. Type system providers need not provide mechanized proofs. Instead, these guarantees require only a form of translation validation that uses type abstraction to check that tycons maintain \emph{translation independence}, analagous to representation independence in ML-style module systems.
\end{abstract}
...
Universality properties (e.g. ``Turing completeness'') are often enough to guarantee that an embedding that preserves a desirable fragment's dynamic semantics can be constructed, but an isomorphic embedding must also preserve the static semantics and, if defined, performance bounds specified by a cost semantics. This is not always possible. For example, in $\mathcal{L}\{{\rightharpoonup}\,{\forall}\,{\mu}\,{\keyw{1}}\,{\times}\,{+}\}$, it is impossible to introduce record types as a library because you need row projection operators, in ML written $\opname{\#}\conclbl{lbl}$, one for each of the infinite set of row labels $\conclbl{lbl}$ ($\opname{\#}$ itself is thus an \emph{operator constructor}). Each time a fragment like must be introduced directly, rather than as a library, a new \emph{dialect} of the language arises. Within the ML lineage, dialects that go beyond ``core ML'' abound:%, showing that this is not an isolated example:
