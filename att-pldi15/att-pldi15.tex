...
\vspace{-3px}
\item
\textbf{Foreign Fragments:} A safe and natural foreign function interface (FFI) can be valuable. This requires enforcing the type system of the foreign language in the calling language. %Using  a FFI that does not do this can lead to safety issues, even when both languages are separately known to be safe. 
...
%In @$\lambda$, the EL builds in only function types. All others arise from the \emph{tycon context}, $\Phi$. 




%The reason is that in the Harper-Stone elaboration semantics, external and internal terms were governed by a common type system, but in @$\lambda$ 
...
To eliminate even these localized failure modes, we plan to introduce \emph{optional} specification and proof mechanization into the SL (by basing it on a dependently typed language like Coq, rather than ML). Because types are values in the SL, the SL can be seen as building on the concept of \emph{type-level computation}\todo{find appropriate citation}. %We also hope to mechanize the metatheory of @$\lambda$ itself in future work.%Every combination of fragments is a new dialect, and must be reasoned about monolithically. %Efforts to reason modularly areeed, several problems can come up when fragments are combined, so . 
