...
As a result, clients can granularly import any necessary primitive constructs when using code that relies on them, and thus achieve full safety, ease-of-use and performance due to the absence of wrappers and glue code. Providers of components thus need only consider whether primitives that they use are appropriate for their domain, without also considering whether their code might be used in a context where these primitives are not otherwise appropriate. Libraries containing logic that is invoked at compile-time have been called \emph{active libraries} \cite{activelibraries}. We diagram this competing approach in Figure \ref{approaches}(b).
...
Types are type-level values of kind $\star$ (following System $F_{\omega}$ \cite{fomega}) and classify expressions, $e$. The type-level language also includes other kinds of terms, such as type-level functions and other type-level data structures. We include lists (required by our mechanism) as well as type-level integers, strings and products for the sake of our examples (see Sec. \ref{termination} for design constraints in general). It also includes constructs for developing extensions, which we will discuss below.
...
\label{rec}&\quad\tops{rec}{\kunit}{i}{a}{
	...
	&\quad\quad \tapp{\tapp{\tapp{\tvar{check\_type}}{\tvar{t3}}}{\ttype{Arrow}{(\ttype{Nat}{\tunit},\ttype{Arrow}{(\tvar{t2},\tvar{t2})})}}}{\\
		...
		&\quad\quad\quad \iIfEq{x}{0}{\dint}{\iup{\tvar{x2}}}{\iapp{\iapp{\iup{\tvar{x3}}}{(x-1)}}{(\iapp{f}{(x-1)})}}}})}{\iup{\tvar{x1}}}}}{\tvar{t2}}
	)}}
	}}}}}}}}}}}}
\\
&}{XXX}{i}{\titype{\dint}};\\
...
\label{recuse}&\quad (\elam{x}{\tvar{nat}}{\elam{y}{\tvar{nat}}{
...
\label{example}
\end{figure}
To make our explanation of each of the constructs in the calculus concrete, we will work through an example showing how to introduce primitive natural numbers with bounded recursion in the style of G\"odel's T \cite{pfpl}. These will be implemented internally as integers (that is, translation will always produce internal terms of internal type $\dint$). Figure \ref{example} shows how the indexed type family $\fvar{Nat}$ is defined. This family is indexed by a unit value (of kind $\kunit$), so it contains only one type, $\ttype{Nat}{\tunit}$, which we alias by defining the type-level variable $\tvar{nat}$. We define the typechecking and translation logic for the operators associated with this family (\opvar{z}, \opvar{s} and \opvar{rec}) and use these to define an add function and compute 2+2.

\subsection{Indexed Type Families and Types}\label{families}
The syntactic form $\familyDf$ declares a new primitive type family named $\fvar{Fam}$ indexed by type-level values of kind $\kappaidx$ (line \ref{natfam}). This can be compared to adding a new constructor to the compiler-internal datatype \lstinline{Type}, as suggested in Sec. \ref{evolution}. The index represents the data associated with the constructor. A base type like $\tvar{nat}$ can be thought of as being the only type in the family $\fvar{Nat}$ trivially indexed by the unit value, of kind $\kunit$, while families like $\fvar{Ntuple}$ might be indexed by a list of types, of kind $\klist{\kTypeBlur}$. A type (that is, a type-level term of kind $\kTypeBlur$) is constructed by naming a family in scope and providing a type-level term of the appropriate kind as an index. For example, $\ttype{Ntuple}{\tcons{\tvar{nat}}{\tcons{\tvar{nat}}{\tnil{\kTypeBlur}}}}$ might be the type of a pair of natural numbers. It is important that type equality be decidable, so only kinds for which equivalence coincides with syntactic equality can be used as type family indices. The main  consequence of this restriction is that indices cannot contain type-level functions.

As we will discuss further below, it is important that all expressions classified by a type compile to internal terms of consistent internal type\todo{mention compiler correctness history here?}. For this reason, we require that every type have an associated internal type. This is computed by substituting the type index for the bound variable $\tvar{i}$ in the term $\taurep$, called the \emph{representation schema}, associated with the type family and reducing to a value representing an internal type. Internal types, $\sigma$, are reified as type-level terms of kind $\kIType$ using the introductory form $\titype{\sigma}$ (as we will see, an  elimination form  is not necessary). 


\subsection{Indexed Operator Families and Denotations}\label{operators}
Type families also have a collection of primitive operator families associated with them. An operator family named $\opvar{op}$ is declared using the form $\tops{op}{\kappaidx}{i}{a}{\tauop}$. Like type families, operator families are indexed by values of some specified kind, $\kappaidx$. Operators are not first-class type-level values in our calculus, so there are no restrictions related to equality. In the example in Fig. \ref{example}, all the operators are trivially indexed by the kind $\kunit$, so each family is only inhabited by one operator. However, a family like $\fvar{Ntuple}$ would have a family of projection operators, $\opvar{pr}$, indexed by a position (e.g. an integer in our calculus). A family implementing record types or object types might have a similar operator indexed by a type-level string representing the field being accessed.

An operator application is an expression of the form $\eop{Fam}{op}{\tauidx}{e_1; \ldots; e_n}$, where $n \geq 0$. The typechecking and translation of an expression of this form is controlled by the declaration of $\opvar{op}$ in family $\fvar{Fam}$, as determined by the type-level term $\tauop$. It must evaluate to a \emph{denotation}, which is a type-level value of kind $\kDen$, when the index, $\tauidx$, and a list containing the denotations recursively assigned to each argument, $e_1$ through $e_n$, are bound to $\tvar{i}$ and $\tvar{a}$, respectively.

There are two forms of denotations. A \emph{valid denotation} has the form $\tden{\tau_1}{\tau_2}$, where $\tau_1$ represents the \emph{translation} of an expression to an internal term and $\tau_2$ is the type. The translation must have kind $\kITerm$ of  reified internal language terms. The only introductory form for this is $\titerm{\gamma}$, just as with $\kIType$. There is no elimination form. Types, as before, have kind $\kTypeBlur$.
...
If a type cannot be assigned to an operation according to the specification of the operator being implemented, then the \emph{error denotation}, $\terr$, should be returned instead of a valid denotation. In a practical implementation, a specialized error message and other diagnostic information could be associated with $\terr$, but we omit such details for the sake of simplicity in our core calculus. Terms of kind $\kDen$ can be \textsf{case} analyzed to determine if they are valid, and if so, to extract the translation and type.
...
The recursor operator, $\opvar{rec}$, proceeds similarly, extracting the translations and types of each of its three arguments. The third argument, the recursive case, binds two variables (the predecessor and the result of recursing on it). In \atlam, the build-in $\lambda$ operator serves as the primitive mechanism for working with bound variables. While lambda terms conventionally have types of the form $\tau_1 \rightarrow \tau_2$, \atlam~instead includes a built-in type family, $\fvar{Arrow}$,  that is always in scope. This family is indexed by a pair of types, $\kpair{\kTypeBlur}{\kTypeBlur}$, and its representation schema simply maps to the corresponding internal arrow type, $\tvar{i}.\titype{\darrow{\trepof{\tfst{\tvar{i}}}}{\trepof{\tsnd{\tvar{i}}}}}$. The special form $\trepof{\tau}$ is used to refer, abstractly, to the representation of another type (we also see this used on line \ref{fix}). Although $\lambda$ is built-in, application is an operator associated with the $\fvar{Arrow}$ family, $\opvar{ap}$, as we see on line \ref{ap}. \todo{figure for arrow types}
...
The first phase ensures that all type-level terms in the program are well-kinded and that all expressions and internal terms are closed. The kinding rules for programs are given in Figure \ref{kindprog}, and they rely on the kinding rules for type-level terms given in Figure \ref{tlkind}. These rules use contexts $\Sigma$ and $\Theta$ to track family and operator signatures but beyond that, these rules are largely consistent with those of a simply-typed lambda calculus, with the addition of the handful of special forms described in the previous sections. The reader is encouraged to verify that the example in Fig. \ref{example} is well-kinded. Active typechecking and translation requires normalizing type-level terms to type-level values at various points, so the kind checking phase ensures that no simple mistakes have been made and that this process will not ``get stuck'' (see Sec. \ref{safety}).
...
\inferrule[op]{
	...
	\opType{\tvarCtx}{\fvarCtx}{{\tops{op}{\kappai}{i}{a}{\tau}}}{
	\kOpS{op}{\kappai}}
}
...
\end{mathpar}
\caption{\small Kinding for programs. Variable contexts $\tvarCtx$ and $\itvarCtx$ obey standard structural properties. Kinding for type-level terms in Figure \ref{kindtl}.}
\label{kindprog}
\end{figure}
\begin{figure}[t]
\small
