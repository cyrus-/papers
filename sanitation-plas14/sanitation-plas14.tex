...
\newcommand{\ecoerce}[2]{{\tt coerce}[#1](#2)}
\newcommand{\estr}[1]{{\tt str\_in[#1]}}
\newcommand{\estrt}[1]{{\tt string\_in[#1]}}
\newcommand{\econcat}[2]{{\tt concat}(#1,#2)}
...
<$\psi$> ::=				\hfill	External types					\\
$\estrt{r}$				\hfill Regular expression types \alt

<T> ::= \hfill External expressions \\
      $\estr{s}$ \alt
      $\econcat{e}{e}$ \hfill String concatenation \alt
      $\ereplace{r}{e}{e}$ \alt
      $\ecoerce{r}{e}$

<$\theta$> ::= \hfill Internal types \\
$\istrt$ \alt $\regext$

<S> ::= \hfill Internal expressions \\
  $\istr{s}$ \alt
  $\econcat{i}{i}$ \alt
  $\ireplace{r}{i}{i}$ \alt
  $\icheck{r}{i}$
...
\onecolumn
\newcommand{\ectx}{?} % Context for external typing
\newcommand{\ictx}{?} % Context for internal typing
...
{\ectx \vdash \estr{s} : \estrt{r}}
{s \in \lang{r}???}

\inferline{Te-Concat}
{\ectx \vdash \econcat{S_1}{S_2} : \estrt{r_1 \cdot r_2}}
{\ectx \vdash S_1 : \estrt{r_1} \\ \ectx \vdash S_2 : \estrt{r_2}}

\inferline{Te-subst}
{\ectx \vdash \ereplace{r}{S_1}{S_2} : r}
{\ectx \vdash S_1 : \estrt{r_1} \\ \ectx \vdash S_2 : \estrt{r_2} \\ r = {\tt lsubst}(r,r_1,r_2)}

\inferline{Te-coerce}
{\ectx \vdash \ecoerce{r'}{s} : \estrt{r'}}
{\ectx \vdash S : \estrt{r}}

\end{tabular}
\caption{Typing rules for $\lcs$. Err cases?}
\label{fig:etyping}
\end{center}
\end{figure}
...
\inferline{Te-Concat}
{\econcat{S_1}{S_2} \ereduces \estr{s_1 \cdot s_2}} % ???
{S_1 \Downarrow \estr{s_1} \\ S_2 \Downarrow \estr{s_2}} 

\inferline{Te-Subst}
{\ereplace{r}{S_1}{S_2} \ereduces s}
{S_1 \Downarrow \estr{s_1} \\ S_2 \Downarrow \estr{s_2} \\ s = {\tt substitution}(r,s_1,s_2)} 

\inferline{Te-Coerce-OK}
{\ecoerce{r}{s} \ereduces \estr{x}}
{s \ereduces \estr{x} \\ x \in \lang{r}}

\inferline{Te-Coerce-NotOk}
{\ecoerce{r}{s} \ereduces \estr{x} \\ x \not \in \lang{r}}
{s \ereduces \estr{x} \\ x \not \in \lang{r}}
\end{tabular}
\caption{Operational semantics for the external language $\lcs$.
There are also unmentioned error propagation rules for for Te-Concat and Te-Subst.}
\label{fig:eeval}
\end{center}
\end{figure}
...
\inferline{Ti-Concat}
{\iconcat{S_1}{S_2} \ireduces \istr{s_1 \cdot s_2}} % ???
{S_1 \ireduces \istr{s_1} \\ S_2 \ireduces \istr{s_2}} 

\inferline{Te-Subst}
{\ireplace{r}{S_2}{S_3} \ireduces {\tt substitution(r,s_2,s_3)}}
{T_1 \ireduces {\tt regex}[r] \\ T_2 \ireduces \istr{s_2} \\ T_3 \ireduces \istr{s_3} \\ } 

\inferline{Te-Ccheck-OK}
{\icheck{r}{s} \ireduces \istr{x}}
{s \ireduces \estr{x} \\ x \in \lang{r}}

\inferline{Te-Check-NotOk}
{\icheck{r}{s} \ireduces \istr{x} \\ x \not \in \lang{r}}
{s \ireduces \istr{x} \\ x \not \in \lang{r}}
\end{tabular}
\caption{Operational semantics for the internal language of $\lcs$.
There are also unmentioned error propagation rules for for Te-Concat and Te-Subst.}
\label{fig:ieval}
\end{center}
\end{figure}
