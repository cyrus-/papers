...
\subsection{Introductory Forms}
On lines 21-30, we define the function \verb|__toplevel__| (the base will consider this a special name for the purposes of compilation, discussed in the Sec. \ref{compilation}). After printing a run-time greeting, we construct a value of type \verb|C| on lines 24-26. Recall that \verb|C| is a record type, so we provide the names of the fields (here, without quotes because we are no longer in the type-level langauge) and values of the appropriate type using syntax for a dictionary literal. Because there may be multiple types that use this form, we say which type the literal should be \emph{analyzed} against with the \emph{literal ascription}  \verb|(C)|. This is another repurposing of Python's syntax: here function application. When the ``function'' has one of Python's literal forms and the (single) argument is a type, we treat it instead as an \emph{ascribed introductory form}. The literal is treated as an introductory form for that type (we will see precisely how this occurs shortly). We see the same form used with tuple literals on lines 27-28 to introduce terms of type \verb|Transfer|. 

\subsection{Bidirectional Typechecking}
The string, number and dictionary forms inside the outermost forms on lines 24-28 are also introductory forms, but we do not include an ascription because the definition of \verb|C| determines which types they should be analyzed against. As we will discuss shortly, Ace has a \emph{bidirectional type system} distinguishing locations where a type must be \emph{synthesized} from an expression (e.g. at the top-level) and locations where an expression is \emph{analyzed} against a type \cite{LovasPfenning08}. The base determines the type a literal should be analyzed against in a synthetic context. For example, if we did not specify the literal ascription \verb|(C)| on line 26, the base would analyze it against the Python dictionary type, which would cause a type error because the variables used as keys were not defined in the surrounding context.
