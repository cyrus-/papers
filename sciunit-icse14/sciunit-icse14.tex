...
\end{enumerate}
But while software engineers can rely on a program's test suite, scientists today must extract this information from a body of scientific publications. This is increasingly difficult. Each publication focuses on just one model and is frozen in time, so it does not consider the latest experimental data or statistical methods. Discovering, precisely characterizing and comparing several models to discover the state of the art, and finding open modeling problems, often requires an encyclopedic knowledge of the literature. Senior scientists can attempt to summarize this with review papers, but in many areas, the number of publications generated every year can be overwhelming \cite{jinha_article_2010}, and comprehensive reviews are published relatively infrequently for any given area. Statisticians commonly complain that scientists are not following best practices and that community standards evolve too slowly because canonical papers or popular reviews advocate outdated methods \cite{grumpystatisticians}. One might compare this to a  software development team relying primarily on carefully-prepared minutes of code review sessions, peppered with cross-references to previous minutes, to understand and validate a complex software project.

We suggest that scientific model validation could benefit from the adoption of a test-driven methodology. However, scientific communities differ from typical development teams in several important ways. 

- Unit tests are typically pass/fail, while goodness-of-fit is typically measured more granularly. 
- Different communities use different goodness-of-fit measures. These continuously evolve as, for example, statisticians develop better methods.
- Professional software developers are used to testing, while scientists are professional end-users and rarely use testing, even for verification. 
