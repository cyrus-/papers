...
For these reasons, most software development teams supplement code reviews with a number of more {automated} approaches to verification and validation. One of the most widely-adopted approaches is \emph{unit testing}. In brief, unit tests are functions that check a single component of a program against a single well-defined correctness criterion \cite{beck2003}. A suite of such tests can be seen as a partial specification of a program or component.  Test suites complement code review by allowing developers to answer questions like these more easily:
\begin{enumerate}
\item How much development progress has been made? Which functionality has not yet been adequately implemented? How valuable is a code contribution?
\item What modes of usage does the team consider most important? How does the development team measure success?
\item Does a code contribution cause \emph{regressions} in other parts of a program?
\end{enumerate}
Scientists ask analagous questions:
\begin{enumerate}
\item When new experimental observations are made, scientists want to determine the impact they have on the validity of previously-developed models.
\end{enumerate}
Answering these questions is often difficult using scientific publications alone. Each publication is frozen in time, so its examination of validity cannot consider the most recent body of data, or modern validation criteria. Discovering these validation criteria, as well as precisely characterizing open modeling problems  requires an encyclopedia knowledge of the literature. In many areas, the number of publications generated every year can overwhelm even the most conscientious and experienced scientists \cite{jinha_article_2010}. A common complaint of statisticians is that scientists are not aware of best practices and that community standards evolve too slowly \cite{grumpystatisticians}. 

Unfortunately, there are few alternatives to comprehensive literature review available when scientists need to answer questions like these today.%Anecdotally, one of the greatest fears of new scientists is that a problem that they are working on has already been solved and published. 
