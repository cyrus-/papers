...
\title{SciUnit: Collaborative Infrastructure for Test-Driven\\Scientific Model Validation}
...
class PositionTest(sciunit.Test):
    ...
    Parameters: 
      observation : ['histories':histories, 'positions':positions]
	...
	def generate_prediction(self, model):
		predictions = []
		for obs_history in self.observation['histories']:
			predictions.append(model.predict_next_pos(obs_history))
		return predictions

	def compute_score(observation, prediction):
		obs_positions = observation['positions']
		p = pooled_p_val(predictions, obs_positions)
		return sciunit.scores.PValue(p, related_data={
			'obs_histories': observation['histories'],
...
A test takes one required parameter in its constructor (constructors are named \verbx{\_\_init\_\_} in Python, lines 7-9), summarizing observed data against which the test will validate models. 
For the class \verbx{PositionTest}, this observation parameter is a dictionary containing two kinds of observations.   
The meaning of this parameter along with a description of the goodness-of-fit metric used by the test is documented on lines 2-6. 
...
\begin{python}
  ...
  saturn_position_test = SpikeCountTest({'histories':h, 'positions':p)
\end{python}
...
Classes that implement the \verbx{sciunit.Test} interface must contain a \verbx{generate_prediction} method that receives a candidate \emph{model} as input and produces a prediction as output. 
They must also contain a \verbx{compute_score} method that can take this prediction, and the observation used to instantiate the test, and produce a \verbx{score}.  
