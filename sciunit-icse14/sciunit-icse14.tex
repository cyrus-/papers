...
Quantitative models have much in common with software. Indeed, quantitative models are increasingly  implemented as computer programs. In some cases the program implements a mathematical model, while in other cases the program \emph{is} the model (e.g. complex simulations). The peer review process is similar in many ways to the \emph{code review} process used by many development teams \cite{codereview}. During code review, team members look for errors, enforce style and architectural guidelines and check that the code is valid (i.e. that achieves its intended goal \cite{validity}) before it is committed to a primary source code repository. 
...
For these reasons, software development teams typically supplement code reviews with a number of more \emph{automated} approaches. One of the most widely-adopted approaches is \emph{unit testing}.
