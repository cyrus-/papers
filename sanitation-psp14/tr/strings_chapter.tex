...
% \begin{trlem}[Correctness of Head] \label{trlemmax}
% If $c_1  s' \in \lang{r}$, then
% $c_1 \in {\lhead{r,\epsilon}}$.
% \end{trlem}
% \begin{proof}
% By structural induction on $r$.

% The case for $r = \epsilon$ does not apply because $s$ is non-empty.

% If $r = a$ for some character $a$ then $s \in \lang{r}$ implies $c_1 = a$.
% By definition $\lhead{a, \epsilon} = \{a\}$.
% Finally, $a \in \{a\}$ by definition.

% If $r = r_1 \cdot r_2$ then we proceed by case analysis on $r_1$. 
% \begin{quote}
% We need not consider the case where $r_1=\epsilon$ by congruence.

% If $r_1=a$ then $c_1=a$ and $\lhead{a \cdot r_2, \epsilon}=\lhead{a, r_2}=a$ and $a \in \{a\}$ by definition.

% If $r_1=r_{11}\cdot r_{12}$, then we have that $\lhead{r_{11} \cdot r_{12} \cdot r_2, \epsilon}=\lhead{r_{11}, r_{12}\cdot r_2}$. By the IH we have that $c_1 \in {\lhead{r_{11}\cdot r_{12}, \epsilon}}$. By Lemma \ref{trlemma:head-concatenation-1}, we have that $c_1 \in \lhead{r_{11}\cdot r_{12} \cdot r_2, \epsilon}$.

% If $r_1=r_{11} + r_{12}$, then by definition we have that 
...
\item \textbf{Case SP-E-ConcatLeft}.
Suppose:
\begin{align*}
  \iota &= \concat{\iota_1}{\iota_2} \mapsto \concat{\iota_1'}{\iota_2} \\
  \emptyset &\vdash \iota : \str \\
  \iota &\mapsto \iota' 
\end{align*}

The only appliable typing rule is P-T-Concat, so $\emptyset \vdash \iota_1 : \str$
and $\emptyset \vdash \iota_2 : \str$ where $\tau = \str$. 
By induction, $\emptyset \vdash \iota_1' : \str$,
so $\emptyset \vdash \concat{\iota_1'}{\iota_2} : \str$.

\item \textbf{Case SP-E-ConcatRight}
\begin{align*}
  e &= \concat{e_1}{e_2} \mapsto \concat{e_1}{e_2'} \\
...
%\begin{itemize}[label=$ $,itemsep=1ex]
%\end{itemize}
>>>>>>> 15fe1f3b63a907df19382ffc8489b4dfeb87a7e1
\end{proof}
...
\begin{mathpar}
\inferrule[PP-E-ConcatLeft]
{\iota_1 \mapsto \iota_1'}
{\tconcat{\iota_1}{\iota_2} \treduces \tconcat{\iota_1'}{\iota_2}}

\inferrule[PP-E-ConcatRight]
{\iota_2 \mapsto \iota_2'}
{\tconcat{\iota_1}{\iota_2} \treduces \tconcat{\iota_1}{\iota_2'}}

\inferrule[PP-E-Concat]
{\ }
{\tconcat{\tstr{s_1}}{\tstr{s_2}} \treduces \tstr{s_1s_2}}

\inferrule[PP-E-CaseLeft]
{ \iota_1 \mapsto \iota_1' }
{ \pstrcase{\iota_1}{\iota_2}{x,y.\iota_3} \mapsto \pstrcase{\iota_1'}{\iota_2}{x,y.\iota_3} }


\inferrule[PP-E-CaseRight]
{ \iota_2 \mapsto \iota_2' }
{ \pstrcase{\dot{v}}{\iota_2}{x,y.\iota_3} \mapsto \pstrcase{\dot{v}}{\iota_2'}{x,y.\iota_3} }

\inferrule[PP-E-Case]
{ }
{ \pstrcase{\tstr{s_1}}{\tstr{s_2}}{x,y.\iota_3} \mapsto \tstr{s_1s_2}}


\inferrule[PP-E-ReplaceLeft]
{\iota_1 \mapsto \iota_1'}
{ \preplace{\iota_1}{\iota_2}{\iota_3} \mapsto \preplace{\iota_1'}{\iota_2}{\iota_3} }

\inferrule[PP-E-ReplaceMid]
{\iota_2 \mapsto \iota_2'}
{ \preplace{\rx{r}}{\iota_2}{\iota_3} \mapsto \preplace{\rx{r}}{\iota_2'}{\iota_3} }

\inferrule[PP-E-ReplaceRight]
{\iota_3 \mapsto \iota_3'}
{ \preplace{\rx{r}}{\tstr{s_2}}{\iota_3} \mapsto \preplace{\rx{r}}{\tstr{s_2}}{\iota_3'} }

\inferrule[PP-E-Replace]
{\ }
{ \preplace{\rx{r}}{\tstr{s_2}}{\tstr{s_3}} \mapsto \tstr{\lsubst{r}{s_2}{s_3}} }

\inferrule[PP-E-Check-Left]
{ \iota_x \mapsto \iota_x' }
{ \check{\iota_x}{\iota}{\iota_1}{\iota_2} \mapsto \check{\iota_x}{\iota}{\iota_1}{\iota_2} }

\inferrule[PP-E-Check-Right]
{ \iota \mapsto \iota' }
{ \check{\rx{r}}{\iota}{\iota_1}{\iota_2} \mapsto \check{\rx{r}}{\iota'}{\iota_1}{\iota_2} }

\inferrule[PP-E-Check-OK]
{ s \in \lang{r} }
{ \check{\rx{r}}{\tstr{s}}{\iota_1}{\iota_2} \mapsto \iota_1}

\inferrule[PP-E-Check-OK]
{ s \not\in \lang{r} }
{ \check{\rx{r}}{\tstr{s}}{\iota_1}{\iota_2} \mapsto \iota_2}
