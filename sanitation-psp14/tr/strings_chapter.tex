...
\todo{Everything above in this proof was sleep deprived plane work -- need to check}

\item \textbf{Case PS-E-Concat}
Let $e = \rconcat{\rstr{s_1}}{\rstr{s_2}} \mapsto \rstr{s_1s_2}$. 
The only rule that applies is P-T-Concat, so $\theta \vdash e : \str$.
By canonical forms, $\theta \rstr{s_1s_2} : \str$.
...
The only rule that applies is P-T-Case, so $\theta \vdash \iota : \tau$ where:
\begin{align*}
  \theta &\vdash \iota_1 : \str 
  \theta &\vdash \iota_2 : \tau
  \theta, x : \str, y : \str &\vdash \iota_3 : \tau
\end{align*}
By induction, $\theta \vdash \iota_1' : \str$.
By P-T-Case, $\thet \vdash \strcase{\iota_1}{\iota_2}{x,y.\iota_3} : \tau$.

\item \textbf{Case PS-E-CaseRight}
Let $\iota = \strcase{\iota_1}{\iota_2}{x,y.\iota_3} \mapsto \strcase{\iota_1}{\iota_2}{x,y.\iota_3}$
when $\iota_1 \mapsto \iota_1'$.
The only rule that applies is P-T-Case, so $\theta \vdash \iota : \tau$ where:
\begin{align*}
