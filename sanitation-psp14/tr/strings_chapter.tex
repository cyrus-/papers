...
\maketitle
\section{String and Language Replacement}
...
presented in \cite{fulton2014}. In addition, we provide some examples to help 
...
\begin{trexam}[All the heads \\
of all the tails \\
can be more than \\
one head and tail]\label{exam:headsandtails}
...
\todo{clean up these paragraphs}
Example~\ref{exam:headsandtails} does not cause type soundness problems for ${\sf strcase}$ because $s \in \lang{r} \implies s \in \lhead{r}\cdot\ltail{r}$ is the
property required for soundness. Still, in a production implementation, it will make sense to massage the definitions of $\lhead{r}$ and $\ltail{r}$ so that type information is not unnecessarily lost during sub-string operations.

This is a general pattern in string operations -- $\lambdas$ simulates common operations on strings within the type system.
If there is an operation for concatenating to strings, we define an operation for concatenating to regular expressions.
If there is an operation for peeling off the first (n) characters of a string, then we define an operation for peeling off
the first (n) characters of a regular expression.

It is important to note, however, that we need not \emph{exactly} simulate the action of string operations using regular expressions.
In the case of concatenation, we lose some information -- of course the string ad is never in $r$, but after decomposing the string, our
type tells us this might actually be the case. That's okay, because the types are conservative in their approximation. Soundness is never violated.
...
the effect of a substitution. Closing these gaps in approximation is important, and motivates the string operations portion of this TR.

\subsection{Some Corollaries About Substitution and Language Replacement}

\begin{defn}[$\tt{subst}$]
We consider several choices in the previous section.
\end{defn}
...
In the S-T-Concat case, note that \ref{thm:total} implies that if $r_1$ and $r_2$ are regular expressions then so is $r_1 \cdot r_2$.
...
To prove type safety and the security theorem for the big step semantics, we
first prove type safety for a small step semantics in Figure~\ref{sdlambdas}
and then extend this to the big step semantics in Figure~\ref{dlambdas} by
...
Note that small step evaluation for $\lambdas$ is terminating:
...
If $\vdash e : \sigma$ either $e \ \val$ or $e \mapsto e'$ for some $e'$.
\end{trlem}
\begin{proof}
The proof proceeds by induction on the derivation of $\vdash e : \sigma$.
...
\item \textbf{S-T-Stringin-I}. Suppose $\vdash \rstr{s} : \stringin{s}$.
The $\rstr{s} \ \val$ by SS-E-RStr.

\item \textbf{S-T-Concat}. Suppose $\vdash \rconcat{e_1}{e_2} : \stringin{s}$.
...
and $e_2 = \rstr{s_2}$.
Note that $\rconcat{\rstr{s_1}}{\rstr{s_2}} \mapsto \rstr{s_1s_2}$ by SS-E-Concat.

\def\mline#1{\par\hspace*{-\leftmargin}\parbox{\textwidth}{\[#1\]}}
\item \textbf{S-T-Case}.
Suppose $e = \strcase{e_1}{e_2}{x,y.e_3}$.
By inversion, $e_1 : \stringin{r}$.
%\begin{align}
...
From this fact, induction, and canonical it follows that $e_1 \mapsto e_1'$ or $e_1 = \rstr{s}$.
...
Suppose $e = \rreplace{r}{e_1}{e_2}$ and $e : \stringin{r'}$ where, by inversion of S-T-Replace,
\begin{align}
&\vdash e_1 : \stringin{r_1} \\
&\vdash e_2 : \stringin{r_2} \\
...
In this case, $e \mapsto \rstr{\subst{r}}{s_1}{s_2}$ by SS-E-Replace.

\item \textbf{S-T-SafeCoerce}.
Suppose that $\vdash \rcoerce{r}{e_1} : \stringin{r}$.
...
Suppose that $\vdash \rcheck{r}{e_1} : \stringin{r}$.
...
Otherwise, by canonical forms $e_0 = \rstr{s}$.
By Lemma~\ref{thm:regexprops}, $s \in \lang{r_0}$ or else not.
In either case, the SS-E-Check-Ok and SS-E-Check-NotOk steps $e$.
...
Suppose $e = \rconcat{e_1}{e_1} \mapsto \rconcat{e_1'}{e_2}$.
By inversion of S-T-Concat, $\vdash e_1 : \stringin{r_1}$ where $\vdash e : \stringin{r_1r_2}$.
By induction if $e_1 \mapsto e_1'$ then $\vdash e_1' : \stringin{r_1}$.
Therefore, $\vdash \rconcat{e_1'}{e_2} : \stringin{r_1r_2}$.

\item {\textbf{SS-E-Concat-Right}}. Similar to SS-E-Concat-Left.

\item \textbf{SS-E-Concat}. Suppose $\rconcat{\rstr{s_1}}{\rstr{s_2}} : \stringin{r_1r_2}$ and
$\rconcat{\rstr{s_1}}{\rstr{s_2}} \mapsto \rstr{s_1s_2}$.
Then by inversion $\rstr{s_1} : \stringin{r_1}$ and similarly for $\rstr{s_2}$.
Therefore, $s_1 \in \lang{r_1}$ and $s_2 \in  \lang{r_2}$ from which it follows by Lemma~\ref{thm:regexprops}
that $s_1s_2 \in \lang{r_1r_2}$. Therefore, $\vdash \rstr{s_1s_2} : \stringin{r_1r_2}$ by S-T-Rstr.

\item \textbf{S-E-Case-Left}.
Suppose that $e = \strcase{e_1}{e_2}{x,y.e_3} \mapsto \strcase{e_1'}{e_2}{x,y.e_3}$
and $\emptyset \vdash e : \stringin{r}$.
By inversion of S-T-Case:
\begin{align}
&\vdash e_1 : \stringin{r} \label{align7} \\
&\vdash e_2 : \sigma \label{align8} \\ 
x:\stringin{\lhead{r}}, y : \stringin{\ltail{r}} &\vdash e_3 : \sigma \label{align9}
\end{align}
By (\ref{align7}) and the assumption that $e_1 \mapsto e_1'$, it follows by induction that $e_1' : \stringin{r}$.
This fact together with (\ref{align8}) and (\ref{align9}) implies by S-T-Case that
$\strcase{e_1'}{e_2}{x,y.e_3} : \sigma$.
...
By inversion of S-T-Case:
\begin{align}
&\vdash e_1 : \stringin{r} \label{align10}\\
&\vdash e_2 : \sigma \label{align11} \\ 
x:\stringin{\lhead{r}}, y : \stringin{\ltail{r}} &\vdash e_3 : \sigma \label{align12}
\end{align}
By (\ref{align11}) and the assumption that $e_2 \mapsto e_2'$, it follows by induction that $e_2' : \stringin{r}$.
This fact together with (\ref{align10}) and (\ref{align12}) implies by S-T-Case that 
$\strcase{e_1}{e_2'}{x,y.e_3} : \sigma$.

\item \textbf{SS-E-Case-$\epsilon$-Val}.
Suppose $e = \strcase{-}{e_2}{-}: \sigma$ and $e \mapsto e_2$.
...
By inversion of S-T-Case:
\begin{align}
&\vdash \rstr{as} : \stringin{r} \label{align13}\\
&\vdash \rstr{e_2} : \sigma \label{align14} \\ 
...
Therefore, $a \in \lhead{r}$ by definition of ${\sf lhead}$.
...
From these facts about $a$ and $s$ we know by S-T-Rstr that $\vdash \rstr{a} : \stringin{\lhead{r}}$
and $\vdash \rstr{s} : \stringin{\lhead{r}}$.
It follows by (\ref{align15}) that  $\vdash [\rstr{a}, \rstr{s} / x,y] e_3 : \sigma$.
...
By inversion of S-T-SafeCoerce we know that $s \in \lang{r}$ from which it follows by S-T-Rstr that $\emptyset\vdash s : \stringin{r}$.

\item \textbf{Case SS-E-Check-Ok}.
Suppose that $\rcheck{r}{\rstr{s}}{x.e_1}{e_2} \mapsto \subst{\rstr{s}}{x}{e_1}$,
$s \in \lang{r}$ and that $\emptyset \vdash \rcheck{r}{\rstr{s}}{x.e_1}{e_2} : \sigma$.
...
Suppose that $\rcheck{r}{\rstr{s}}{x.e_1}{e_2} \mapsto e_2$,
$s \not \in \lang{r}$ and that $\emptyset \vdash \rcheck{r}{\rstr{s}}{x.e_1}{e_2} : \sigma$.
...
Before extending these the type safety result fir the small step semantics to the big step semantics, we first establish a correspondence between
the big step semantics in Figure~\ref{fig:sdlambdas} and the small step semantics in Figure~\ref{fig:dlambdas}.
We prove the relevant theorems for the $\mapsto$ relation because these proofs are easier. We then extend this result via the correspondence theorem
to the more concise big step semantics presented in \cite{fulton2014}.
...
From these two facts we know via TR-Step that $\rreplace{r}{e_1}{e_2} \mapsto^* \rreplace{r}{e_1}{e_2}$.
...
If $e \vdash \tau$, $e \mapsto^* v$ and $v \ \val$ then $e \sreduces v$.  
\end{trthm}
\begin{proof}
...
By inversion, $\vdash e_1 : \stringin{r_1}$.
...
and so $e \mapsto^* \rstr{s_1s_2}$ by TR-Step. So it suffices to show that $e \sreduces \rstr{s_1s_2}$.
...
\item \textbf{Case $e = \rreplace{r}{e_1}{e_2}$}.
By inversion of S-T-Replace, $\vdash e_1 : \stringin{r_1}$ for some $r_1$.
...
and so $e \mapsto^* \rstr{\lsubst{r}{s_1}{s_2}}$ by TR-Step. 
...
similarly for $e_2 \sreduces \rstr{s_2}$.


\item \textbf{Case $e = \strcase{e_1}{e_2}{x.y.e_3}$}.
By inversion, $\vdash e_1 : \stringin{r}$ and $e_2 : \sigma$.
By type safety, canonical forms and termination $e_1 \mapsto^* \stringin{s_1}$ and by induction $e_1 \sreduces \stringin{s_1}$.
Similarly, $e_2 \mapsto^* v_2$ and $\vdash e_2 \sreduces v_2$.

By SS-E-Case-LR*, $\strcase{e_1}{e_2}{x.y.e_1} \mapsto^* \strcase{v_1}{v_2}{x.y.e_2}$.
...
Also, $e_1 \mapsto \strin{\epsilon}$ and $e_2 \mapsto v_2$ is enough to establish by S-E-Case-$\epsilon$ that $e \sreduces v_2$.
...
\end{thm}
\begin{proof}
If $\emptyset \vdash e : \sigma$ then $e \mapsto^* v$ for $v \ \val$ by termination and type safety for the small step semantics.
...
\end{thm}
\begin{proof}
If $\emptyset \vdash e : \stringin{r}$ then $\emptyset \vdash \rstr{s} : \stringin{r}$ by type safety.
