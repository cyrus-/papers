...
\abstract{This report contains supporting evidence for claims put forth and explained in the paper ``Statically Typed String Sanitation Inside a Python'' \cite{sanitation-psp14}, including proofs of lemmas
...
Theorems and lemmas appearing in \cite{sanitation-psp14} are numbered correspondingly, while supporting facts appearing only in the Technical Report are lettered.
Throughout this technical report, we use a small step semantics corresponding to the big step semantics given in \cite{sanitation-psp14}.
...
The translation from $\lambdas$ to $\lambdap$ is specified in Figure \ref{fig:tr}.
...
\begin{defn}[Multistep] We write $\iota \mapsto^* \iota'$ for the reflexive, transitive closure of the stepping judgement. \end{defn}
...
\item \textbf{Case SS-E-Case-Epsilon}. 
...
\item \textbf{Case SS-E-Case-Cons}.
...
%   If $\sctx \vdash e : \sigma$ then 
%   $\trden{\sctx} \vdash \trden{e} : \trden{\sigma}$.
%   Furthermore, if $e \mapsto^* v$ then
%   $\trden{e} \mapsto^* \trden{v}$.
% \end{thm}
% \begin{proof}
% By Theorem 8 and iterated application of Theorem 9.
%
...
%   If $\emptyset \vdash e : \stringin{r}$ and $e \mapsto^* \rstr{s}$ then $\trden{e} \mapsto^* \tstr{s}$
%   and $s \in \lang{r}$.
% \end{thm}
% \begin{proof}
