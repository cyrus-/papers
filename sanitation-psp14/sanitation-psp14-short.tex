...
% Our type system for regular string types, $\lambdas$, is defined as an exntension to a simply-typed lambda calculus.
% Operations on strings are given types according to corresponding, type-level
% computations on regular expressions. A concatenation operation is defined in 
% order to demonstrate this principle, but the premier operation of our system
...
% to existing input sanitation techniques that use regular expressions for input sanitation.
...
\small
$\fbox{\inferrule{}{\sctx \vdash S : \sigma}}$
...
\inferrule[S-T-App]
{\sctx \vdash e_1 : \sigma_1 \\ \sctx \vdash e_2 : \sigma_1 \rightarrow \sigma}
...
{e \sreduces \strin{s} \\ s \in \lang{r}}
{
  \rcheck{r}{e}{x.e_1}{e_2} \sreduces [\strin{s} / x]e_1
}
...
In this section, we define a minimal typed lambda calculus with regular string types called $\lambda_{RS}$. This will serve as the source language for our translation to a calculus with only standard strings and regular expressions, defined in Sec. \ref{translation}. 

The syntax of $\lambda_{RS}$ is given in Figure \ref{fig:lamrs}, its static semantics are specified in Figure \ref{fig:etyping} and its evaluation semantics are specified in Figure \ref{fig:eeval}. 
...
{\sctx \vdash \trden{e_1e_2} = \iota_1\iota_2}
...
\inferrule[Tr-Concat]
{ \sctx \vdash \trden{e_1} = \iota_1 \\ \sctx \vdash htrden{e_2} = \iota_2}
...
{ \ }
{ \sctx \vdash  \trden{ \coerce{r'}{e} } = \trden{e} }

\inferrule[Tr-Check-Ok]
{ \sctx \vdash e : \stringin{r} \\ \lang{r} \subseteq \lang{r'} } 
{ \sctx \vdash  \trden{ \rcheck{r'}{e}{x.e_1}{e_2} } = \trden{[e/x]e_1} }

\inferrule[Tr-Check-NotOk]
{ \sctx \vdash e : \stringin{r} \\ \lang{r} \not \subseteq \lang{r'}}
{ \sctx \vdash  \trden{ \rcheck{r'}{e}{x.e_1}{e_2} } = \trden{\tcheck{r'}{e_2}} }
...
The translation is type-directed in the Tr-SafeCoerce case.}
\end{figure*}
...
\begin{mathpar}
\inferrule[P-T-Abs]
{\tctx, x : \iota_1 \vdash e : \iota_2}
{\tctx \vdash \lambda x.e : \iota_1 \rightarrow \iota_2}

\inferrule[P-T-App]
{\tctx \vdash e_1 : \iota_1 \\ \tctx \vdash e_2 : \iota_1 \rightarrow \iota}
{\tctx \vdash e_1 e_2 : \iota}
...
\inferrule[P-T-Check]
{\tctx \vdash \iota_1 : \regex \\ \tctx \vdash \iota_2 : \str \\ }
{\tctx \vdash \tcheck{\iota_1}{\iota_2} : \str}
\end{mathpar}
...
{\iota_1 \treduces \rx{r} \\ \iota_2 \treduces \strin{s} \\ s \in \lang{r}}
{\tcheck{\iota_1}{\iota_2} \treduces \tstr{s}}

\inferrule[P-E-Check-Err]
{\iota_1 \treduces \rx{r} \\ \iota_2 \treduces \tstr{s} \\ s \not \in \lang{r}}
{\tcheck{\iota_1}{\iota_2} \err}
\end{mathpar}
...
In the previous section, we specified a type system and a translation semantics to a language containing only strings and regular expressions. In this section, we take Python to be such a target language. Python does not have a static type system, however, so to implement these semantics, we will leverage \verb|atlang|, an extensible type system for Python (being developed by the authors). By using \verb|atlang|, we can implement these semantics as a library, rather than as a dialect of Python. 

\begin{figure}\label{fig:atexample}
\begin{lstlisting}
