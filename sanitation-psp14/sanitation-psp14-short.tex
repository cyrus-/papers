...
In addition to safety, we proof a correctness result for $\lambdas$ which will
be preserved under translation.

\begin{thm}{Input Sanitation Correctness of $\lambdas$}\label{thm:scorrect}
  If $\sctx \vdash S : \stringin{r}$ then either (a) $S \sreduces \rstr{s}$ for $s \in \lang{r}$
  or (b) $S \err$.
\end{thm}
\begin{proof}
  We may actually want a proof here.
\end{proof}

Finally, $\lambdap$ is also type-safe. We omit the proof due to space constraints,
and because the proof is trivial.
...
\begin{thm}
If If $\sctx \vdash S : \stringin{r}$ and $\trden{S} = P$ then either:
(a) $P \treduces \tstr{s}$ and $S \sreduces \strin{s}$, or                      
(b) $P \err$ and $S \err$. 
\end{thm}
\begin{proof}
  The theorem  follows directly from canonical forms for translations and theorem \ref{thm:tcorrect}.
\end{proof}
...
\begin{thm}{Input Sanitation Correctness for Translated Terms}
  If $\trden{S} = P$ and $\sctx \vdash S : \stringin{r}$ then either $P \err$ or $P \sreduces \tstr{s}$
  for $s \in \lang{r}$.
\end{thm}
\begin{proof}
  By theorem \ref{tcorrect}, $P \sreduces \tstr{s}$ implies that $S \sreduces \strin{s}$.
  By theorem \ref{scorrect}, this together with the assumption that $S$ is well-typed implies that $s \in \lang{r}$.
\end{proof}
