...
\begin{itemize}
  \item In \S 2.1-2.3, we define $\lambdas$, $\lambdap$ and the translation from one to the other.
  \item In \S 2.4 we state some properties about regular expressions which are needed in our correctness proofs.
   \item In \S 2.5 we prove type safety for $\lambdap$ as well as both type safety and correctness for $\lambdas$.
  \item In \S 2.6 we prove that translation preserves the correctness result about $\lambdas$.
\end{itemize}
...
\begin{itemize}
  ...
  \item The check rules allow casts between strings that cannot be guaranteed at compile time, but might pass a runtime check.
\end{itemize}
...
\begin{figure}\label{fig:lambdas}
\small
...
\begin{figure}[h]\label{fig:trans}
...
\end{lstlisting}
\vspace{-10px}
\caption{Regular string types in atlang, a library that enables static type checking for Python. }   \label{examplefig}

\end{figure}

Figure \ref{examplefig} demonstrates the use of string types in atlang.
