...
\item Section \ref{sec:tr} describes the translation from $\lambdas$ to $\lambdap$ and ensures the correctness result for \ref{sec:rs} is preserved under translation. 
\end{itemize}
...
In terms of strings, \textsc{S-T-Case} is essentially the ubiquitious substring operation.
...
Fortunately, regular expression derivatives \cite{bowzer} conveniently capture this intuition.
...
The \textsc{S-T-Case-Concat} rule, which is defined in terms of these operations, relates the result of ``peeling off" the first character of a string to regular expression derivatives.

\subsubsection{Coercion}

The $\lambdas$ language supports two forms of coercion. Safe coercion only allows coercion between 
...
\textsc{S-T-Check} allows casts between strings that cannot be guaranteed at compile time, but inserts a runtime check.

An alternative to coercion is subtyping for constrained string types, which applies coercion implicitly. 
In practice, subtyping is crucial to the usability of our system due to the nature of the replacement operator.
For simplicity, we do not include this relation; however, subtyping for constrained strings is present in \cite{fulton12,fulton13} and that treatment extends to this system. The implementation discussed in Section \ref{atlang} provides subtyping between constrained string types.
...
The definition of \textsc{S-T-Replace} requires a definition of replacement for regular expressions (or languages) corresponding to string replacement.
...
In an analogous manner, the function $\lreplace{r}{r_1}{r_2}$ replaces every sublanguage of $r_1$ matching $r$ with $r_2$.
If the intuition for this operation is not clear, it may be helpful to think in terms of replacing sub-automata.
...
\begin{prop}[Substitution Correspondence] \label{thm:substcorrespondence}
  ...
  $\lsubst{r}{s_1}{s_2} \in \lang{\lreplace{r}{r_1}{r_2}}$.
\end{prop}
\begin{proof}
...
\begin{thm}[Type Safety.] \label{thm:scorrect}
  ...
  then $e \sreduces v$ and $\sctx \vdash v : \sigma$.
\end{thm}
\begin{proof}
...
This is beneficial because safe subsets of strings may be characterized by regular expressions.
Our main technical result, stated later in this section, establishes that the security property
...
\end{thm}
\begin{proof}
The proof proceeds by induction on the typing relation for $e$ and an appropriate choice of $\iota$; in each case,
the choice is the syntactic form in $\lambdap$ corresponding to the form under consideration (e.g. choose {\sf replace} when considering {\sf sreplace}).
...
\begin{figure}[h]\small\begin{lstlisting}
