...
For example, in this case, the developer (or, more often, a framework) might define a function \verb|sanitize| that 
%escapes any double quotes, which are necessary in order to force uesr input to be interpreted as anything other than a string literal.
...
Our type system for regular string types, $\lambdas$, is defined as an exntenstion to a simply-typed lambda calculus.
Opeartions on strings are given types according to corresponding, type-level
computations on regular expressions. A concatenation operation is defined in 
order to demosntrate this principle, but the premier operation of our system
is \emph{replacement}, which captures a rich set of input sanitation techniques
including escaped characters and encodings. In addition to defining this system,
we present a correctness-preserving compilation of $\lambdas$ into an underlying
language with ordinary strings as a regular expression library.

In addition to this theoretical contribution, we also explain how our extension
may be implemented in the Ace Programming Language, which is itself an extension
of Python -- a popular web programming language.

Summarily, we present a simple type system extension which ensures the absence
of input sanitation vulnerabilities by statically checking the correctness and 
correct use of input sanitation algorithms. This approach is \emph{composable}
in the sense that it is a conservative extension. This approach is \emph{complementary}
to exsiting input sanitation techniques that use regular expressions for input santiation.
...
The Wyvern programming language provides a general framework for composing lanugage extensions \cite{tsl}\cite{wyvern}.
...
In order to compute the type resulting from aubstitution, we also need to compute
...
\begin{figure*}[t]\label{fig:trans}
...
\begin{itemize}
  ...
  \item Rule S-T-Coerce allows coercion; however, note that this coercion cannot
    invalidate our safety property because we ensure appropraite checks are always
    inserted wherever coercions are used.
\end{itemize}

\subsection{Definition of $\lambdap$}

The system $\lambdap$ is a stright-forward extension of a simply typed lambda claculus
with a string type and a regular epxression tpye. We include two operations which are available in the regular expression
...
Safety for the string fragment is more involved, because it involves validating that
...
\begin{proof}
  ...
  By theorem \ref{thm:scorrect}, the above property together with the assumption that $S$ is well-typed implies that $s \in \lang{r}$.
\end{proof}
...
In this section, we present an example atlang program using our extension together
with our implementation of the extension in Atlang (modulo ${\sf lreplace}$).
Note that in addition to Figure \ref{fig:regex}, regular expressions in atlang
also support complementation using the \^{} symbol. Atlang is implemented in Python,
and shares the syntax of Python (including the argument annotation syntax introduced in Python 3);
however, Atlang is statically typed and supported the definition of type system extensions by 
overriding Python's dynamic dispatch protocol.

An implementation of a similar extension as the one presented here is discussed in \cite{fulton12} and \cite{fulton13}.
Our prior work only supports a special case of replacement where unsafe substrings are completely removed.
...
The result of applying sanitize to input is apped to two functions which
...
methods define our type checker. The atlang type checker or translater defer
