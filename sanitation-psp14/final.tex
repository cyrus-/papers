...
\newcommand{\inferlbl}[3]
			{\inferrule{#3}{#2}{\textsf{\scriptsize{\sc #1}}}}
...
$\lhead{r}$ is defined in terms of the structure of $r$:
{\footnotesize
\begin{align*}
...
\end{align*}} 
\end{defn}
We use a two-argument auxiliary $\lhead{r, r'}$ in the definition of $\lhead{r}$ because when $r = q^*\cdot t$,
$\lhead{r}$ needs to ``remember" $t$ in case $q$ is iterated zero times.
...
The rule 
 \textsc{S-T-SafeCoerce} checks for language inclusion, which we write $\lang{r_1} \subseteq \lang{r_2}$. Language inclusion is decidable. As a result, the rule \textsc{S-E-SafeCoerce} does not need to perform any checks. The rule 
...
For simplicity, we do not include this operator; however, subtyping for regular strings is present in previous treatments of our work \cite{fulton13}. The implementation discussed in Section \ref{atlang} also provides subtyping between regular string types.
...
It behaves analogously to \lstinline{str_replace} in PHP and \lstinline{String.replace} in Java, differing in that the replacement pattern $r$ must be statically given. The evaluation rule \textsc{S-E-replace} is defined in terms of $\lsubst{r}{s_1}{s_2}$. For simplicity, we do not define replacement in this paper; see \cite{psptr}. 

\begin{defn}[$\tt{subst}$]
$\lsubst{r}{s_1}{s_2} = s$ produces a string $s$ in which all substrings of $s_1$ in $\lang{r}$ are replaced with $s_2$.
\end{defn}



The typing rule \textsc{S-T-Replace} requires computing the regular language of the string resulting from this operation given the languages of $s_1$ and $s_2$, written as $\lreplace{r}{r_1}{r_2}={r'}$. 
...
A complete definition of ${\sf lreplace}$ would also give an rewrite system with correctness and termination proofs, which is beyond the scope of this paper. Instead, we provide an abstract definition of the operation and state necessary properties.
...
\begin{lem}[Normalization for String Fragment of $\lambdas$]\label{thm:cfs}
...
\vspace{-25px}\begin{mathpar}\hspace{20px}
...
\section{Implementation in \tt{atlang}}\label{atlang}
...
\newcommand{\lstinlinep}[1]{\lstinline[language=Python,basicstyle=\ttfamily\scriptsize]{#1}}
...
\begin{figure}[t]\scriptsize\begin{lstlisting}
...
\end{lstlisting}
%\vspace{-10px}
\caption{Regular string types in {\tt atlang}}\label{fig:atexample}
\end{figure}
...
class stringin(atlang.Type):
  ...
  # replace, check and strcase omitted. See TR.
...
\end{lstlisting}
%\vspace{-10px}
\caption{Implementation of the \texttt{stringin} type constructor in {\tt atlang}.}
\label{fig:impl}
...
\setlength{\bibsep}{0pt plus .25pt minus .25pt}
{
\vspace*{-1.5mm}
