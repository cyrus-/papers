...
To further demonstrate the flexibility of this mechanism, we continue in Section \ref{usecases} by outlining examples of other, higher-level parallel abstractions, including global address spaces, message passing and functional data parallelism, that can be cleanly implemented using AT\&T. First-class support for each of these has required a new language in the past. AT\&T enables compile-time logic to be coupled with user-defined types, and thus all of these first-class abstractions can safely be used within a single program or workflow.


 Ace can be used as a standalone language via the \verb|acec| compiler and also as an interactive compilation environment. This latter mode of use supports just-in-time specialization and direct invocation of Ace functions from within Python scripts, as well as deep integration between the widely-used \verb|numpy| library (including its internal type system),  OpenCL's host API (by a mechanism also available to other host APIs, such as CUDA's) and the Ace compiler itself. We describe this in Section \ref{compenv}. Achieving this deep integration makes further use of Ace's representation of types.
 
To demonstrate the utility of Ace for realistic scientific workloads, we briefly describe in Section \ref{casestudy} a case study that uses Ace as a compilation environment supporting a scientific simulation framework capable of orchestrating the execution of thousands of realizations of a large stochastic neurobiological circuit model on clusters of GPUs. 

We conclude in Sections \ref{related} and \ref{discussion} with related work and a discussion of some limitations of Ace at the time of writing, as well as a discussion of future work that builds upon the concept of active typing central to Ace.
...
\section{Design and Usage}\label{usage}
