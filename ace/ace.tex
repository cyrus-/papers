...
\begin{abstract}
Researchers developing languages and abstractions for high-performance computing must consider a number of design criteria, including performance, verifiability, portability and ease-of-use. Despite the deficiencies of today's tools and the availability of apparently superior options, end-users have been reluctant to adopt new language-based abstractions. We argue that this can be largely attributed to a failure to consider three critical criteria: continuity, extensibility and interoperability. This paper introduces Ace, a language that incorporates several novel mechanisms and makes principled design choices in order to satisfy this more comprehensive set of criteria. Ace builds upon existing standards in HPC, particularly Python and OpenCL, and it can be modularly extended with abstractions drawn from several previously disparate paradigms. The core innovation supporting this is a novel reification of types as first-class objects at compile-time, a refinement to the concept of active libraries that we call \emph{active types}. We validate our overall design with a case study of a simulation framework enabling the modular specification and efficient execution of ensembles of neural simulations across clusters of GPUs.
