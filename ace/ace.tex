...
The examples in Section \ref{usage} are based on an internalization of the OpenCL kernel programming language as a library. This library is built atop a novel extensibility mechanism that we call \emph{active typechecking and translation} (AT\&T) and detail in Section \ref{att}. This mechanism relies on the core idea of representing types as metalanguage objects. We refer to such types as \emph{active types} by relation to \emph{active libraries} \cite{activelibraries} (see Section \ref{related}). Unlike prior approaches to extensibility that take a language-global view, which hinders interoperability, the AT\&T mechanism guarantees that extensions are composable by construction by limiting their scope to expressions having a type defined by that extension.
...
It is legitimate to ask, however, why dynamically-typed languages are so widely-used in HPC. One reason has to do with the criteria of \textbf{ease-of-use}. Statically-typed languages, particularly those that HPC programmers are exposed to, can be verbose, requiring explicit type annotations on every function and variable declaration,  separate header files, and other declarations that exist to assist the compiler. Cordy identifies the principle of \emph{conciseness} with elimination of
redundancy and the availability of reasonable defaults \cite{cordy1992hints}. The high-level languages used in HPC have, consistent with this principle, avoided most of this overhead. Ace was originally conceived to explore the question: does this require run-time mechanisms, or can one express a statically-typed, low-level language at least as powerful as C without the overhead that is typically associated with it?

Rather than designing a new syntax, or modifying the syntax of C, we chose to directly use, without modification, an existing language, Python's, syntax. This was not an arbitrary choice but rather a key mechanism by which Ace achieves \textbf{continuity}. Python is amongst the most widely-adopted languages in the world, and enjoys similar  ubiquity within HPC specifically \cite{pythonhpc}. By directly adopting Python's syntax, Ace's syntax is immediately familiar and acceptable to a significant portion of it's intended audience. Moreover, any tools that handle Python source code, including parsers, editors, style checkers and documentation generators, can be used without modification for handling Ace code. 
