...
\begin{abstract}
In this paper, we introduce the Ace compilation environment and demonstrate its practicality as a foundational tool for both high-performance computing research and in practice. 
Ace consists of a statically-typed language that shares syntax with Python and uses the Python language itself as a multi-purpose compile-time metalanguage. Unlike standard languages, however, the Ace compiler consists only of a minimal core that delegates control over most aspects of front-end compilation to {\em active libraries} using a novel extension mechanism that we call {\em active type-checking and translation (\ATT)}. Users specify new primitive types and operations by equipping type definitions, which are first-class objects in the metalanguage, with designated methods that the compiler selectively invokes when type checking and translating expressions. Using \ATT, we implement the primitives of the C99 and OpenCL languages as modules within Ace. We then give examples of several first-class parallel abstractions that can be used alongside these basic primitives, demonstrating the foundational role that an actively-typed system like Ace can play for researchers developing language-based programming abstractions.

Ace also supports more conventional forms of structured and unstructured metaprogramming and Ace programs can be compiled and launched from Python directly. We used these features, along with the OpenCL language module, to design a scientific simulation framework that allows users to modularly specify and orchestrate the execution of parameterized families of simulations on clusters of GPUs. This framework has been used to successfully conduct large-scale, high-performance neuroscience simulations, providing  evidence that Ace is also useful in practice today.
\end{abstract}
