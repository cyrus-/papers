...
\title{Active Code Completion:\\Brief Description}
...
We will present our idea of active code completion technique in the context of object instantiation by introducing our current prototype called \textsc{Graphite}. A 10 minutes long video tutorial is going to be played and live demonstration will be provided so that the conference attendees can test Graphite on the Eclipse IDE. The details of the video are described in the following section.
...
\subsection{Basic usage of Graphite}
...
\subsection{Palette Example 1. Color}

The first example palette is the one for \texttt{Color} class as its benefits are quite intuitive. The palette looks similar to those of graphical editors. Users can specify the desired color in many different ways. For example, they can choose one of the colors predefined in the \texttt{Color} class from the palette, or they can input a `\#' character followed by six hexadecimal digits as people normally do in the HTML and CSS files. Whenever they specify a color, they can directly see the resulting color from the color preview box.

\subsection{Palette Example 2. Regular Expression}
...
\subsection{Writing a new palette for a class}
...
\subsection{External association model}

The aforementioned two example palettes are associated to their corresponding classes via external configuration file. They can be edited from the preferences page of Eclipse IDE, and the video will show how to do so.

\subsection{Annotation-based association model}
...
\url{http://www.youtube.com/watch?v=FfCoXQEHGAI}\\
It is longer than 4 minutes requested, so reviewers who wish to review the video may wish to stop after the Color palette has been demonstrated.
