...
\maketitle
\begin{abstract}
Researchers and domain experts propose novel language primitives regularly. Unfortunately, todayÕs statically-typed languages are monolithic: they do not give users the ability to specify the semantics of new primitive types and operators directly. This has led to a proliferation of standalone languages built around a few situationally useful constructs. An alternative to this language-oriented approach is to work in an extensible programming language. Designing an extensibility mechanism that is both expressive and safe has been a challenge Ð extensions must not weaken the global safety properties of the language, compromise the integrity of the compilation process, or interfere with one other, while still admitting a range of useful constructs.

This paper introduces a mechanism called active type-checking and translation (AT\&T) that aims to address these issues. By repurposing type-level computation in a novel way, AT\&T supports a wide range of user-defined semantics over a fixed grammar in a safe and modular manner. In a language supporting AT\&T, primitive types families are represented uniformly at the type level. Each type is equipped with operators, defined using type-level functions, that are invoked specifically during the type-checking and translation of operations relevant to that type. We discuss three points in the design space: (1) a simple judgmental construction of AT\&T designed to distill the essential concepts and admit formal safety theorems, (2) a "real" language called Ace that we use to demonstrate the expressive power of AT\&T in practice, and (3) a language design called Birdie that lifts and extends CoqÕs rich system of dependent types into the type-level, allowing extension developers to fully prove the correctness of extensions statically.
...
%~~~~~~~~\tifeq{\ttypeof{\tvar{d}}}{\tvar{nat}}{\tden{\iop{1}{\mvalof{\tvar{d}}}}{\tvar{nat}}}{\terr\\~~~~}
%	}}{\terr}\\
%}}}{\tvar{x}}}}}
%$
...
\text{family instances}	&	\phi		&	::= & \fvar{fam} \pipe \tfamSpecStd\\
...
\text{family kinds}					& \eta		& 	::=	& \kFamVar{fam} \pipe \kFamStd\\
...
\inferrule{
	...
	\tentailsXt{\psi_0}{\karrow{\kType{\kFamStd}}{\kappa}}\\
...
\inferrule{
	...
	\tentailsXt{\tden{\psiiterm}{\psitype}}{\kDenk{\kFamStd}}
}~(\text{Den}_I)
...
\inferrule{
	...
	\tentailsXt{\ttypeof{\psi}}{\kType{\kFamStd}}
}~(\text{typeof})
...
\inferrule{
	...
	\kentailsX{\kSimple{\kType{\kFamStd}}}
}~(\text{Type})

\inferrule{
	\kentailsX{\kFamStd}
}{
	\kentailsX{\kSimple{\kDenk{\kFamStd}}}
}~(\text{Den})
\end{mathpar}
...
\inferrule{
	...
	\tentailsXt{\topp{\theta}{\tops{id}{\psi}}}{\Topp{\Theta}{\Tops{id}{\kappa}}}
}~(\text{op-seq})
\end{mathpar}
$\fbox{\inferrule{}{\kentailsX{\eta}}}$~
$\fbox{\inferrule{}{\TentailsX{\Theta}}}$
~~~~$\hat{\Theta} := \Toppstd$
\begin{mathpar}
\small
\inferrule{
...
\inferrule{
	\kentails{\fCtx, \fSpec{fam}{\kappaidx}{\hat{\Theta}}}{\Theta'}\\
	\kentails{\fCtx, \fSpec{fam}{\kappaidx}{\hat{\Theta}}}{\karrow{\splat{\kappa}{1}{m}}{\kOp{n}}}
}{
	\kentails{\fCtx, \fSpec{fam}{\kappaidx}{\hat{\Theta}}}{\hat{\Theta}}
}~(\text{Op-Seq})
\end{mathpar}
...
\inferrule{ }{\ientailsX{\iintlit}}~(\text{int-form}^\mu)
...
$\fbox{$\tevalX{\phi}{\phi'}$}$
~~~~$\famEvalCtx ::= \cdot \pipe \famEvalCtx, \ttype{\psiidx}{\tfamSpecStd}$
~~~~$\errCtx ::= \noprob \pipe \prob \pipe \errCtx, \errCtx$
%~~~~$\inferrule{\tevalX{\psi}{\psi'} \\ \prob \notin \errCtx}{\tevalsX{\psi}{\psi'}{\noprob}}$
%  ~~~~$\Sigma ::= \noprob \pipe \prob \pipe \Sigma,\Sigma$~~~~ 
\begin{mathpar}
\small
\inferrule{ }{
	\tevalsX{\tunit}{\tunit}{\noprob}
...
\inferrule{
	\tevalsX{\tfamSpecStd}{\psit{fam}}{\errCtx_0}\\
	\tevalsX{\subst{\psit{fam}}{\fvar{fam}}{\subst{\kFamStd}{\kFamVar{fam}}{\psi}}}{\psi'}{\errCtx_1}
}{
