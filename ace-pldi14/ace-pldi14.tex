...
We will show in Section \ref{backend} that for targets with Python bindings, such as OpenCL, CUDA, C, Java or Python itself, generic functions can be executed directly, without any of the explicit compilation steps in Listings \ref{compscript}-\ref{mapout}.
...
\label{mapout}
\end{codelisting}
Lines 3-5 of Listing \ref{compscript} construct the types used on lines 7-10. In Ace, types are themselves values that can be manipulated at compile-time, in contrast to most  contemporary languages, where user-defined types are simply declared at compile-time but cannot be defined, inspected or passed around programmatically. More specifically, types are instances of a Python class that implements the \verb|ace.ActiveType| interface (see Sec. \ref{extensions}). Python, then, serves as the \emph{type-level language} of Ace. 
Being Python values, types can be assigned to variables for convenience (a construct similar to \verb|typedef| in C or \verb|type| in Haskell). Types, like all compile-time mechanisms in Ace, do not have mutable state and operate in a referentially transparent manner.% These types are all implemented in the \verb|clx| library imported on line 1, none are built into Ace itself.

The type named \verb|T1| on line 7 corresponds to the OpenCL type \verb|global float*|: a pointer to a 32-bit floating point number stored in the compute device's global memory (one of four memory spaces defined by OpenCL \cite{opencl11}). It is constructed by applying \verb|clx.Ptr|, which is an Ace type constructor corresponding to pointer types, to an address space (here, \verb|clx.global_|) and the type being pointed to, \verb|clx.float|, the Ace type corresponding to \verb|float| in OpenCL. 
The \verb|clx| library contains a full implementation of the OpenCL type system. We will return to how types are implemented in Section \ref{att}. We note that we are being more verbose than necessary for the sake of pedagogy. The \verb|clx| library includes a concise shorthand for OpenCL's built-in types: \verb|T1| could be written \verb|clx.gp(clx.f32)|. We also note that Ace is \emph{unopinionated} about issues like memory safety. We will show how to implement safer alternatives to raw pointers in Sec. \ref{examples}, but we allow users to choose a low level of abstraction if they wish (e.g. for compatibility with existing code).

The type \verb|T2| on line 8 does not correspond to an OpenCL type directly. Rather, it is a global pointer to a user-defined primitive type representing complex numbers (integers, in this case). OpenCL does not include primitive support for complex numbers (as of this writing) 
...
Running \verb|acec listing3.py| would produce no output. To create a {\em concrete function} (that is, an instance of the class \verb|ace.ConcreteFn|) that can be emitted by the compiler, types must be assigned to each of the arguments. Listing \ref{compscript} shows how to use the \verb|compile| method to specialize \verb|map| in \emph{two} different ways to apply the \verb|add5| function, defined on Lines 4-6, to arrays  that reside in global memory (OpenCL associates a memory space with pointer types). Line 9 results in a version specialized for arrays of \verb|double|s and Line 10 results in a version for arrays of \verb|int|s. The output of compilation is shown in Listing \ref{mapout}.
...
\subsection{Programmatic Compilation}\label{procedural}
...
Before moving on to more interesting examples, let us discuss the \verb|acec| compiler shown operating at the shell in Listing \ref{helloout}. 
