...
In this paper, we introduce \Ace, a language compatible with the tools and infrastructure developed for Python, among the most  widely-adopted dynamically-typed language today. While Python and other similar languages have proven satisfactory for small-scale scripting tasks, \Ace~is designed for situations where correctness and performance are particularly important, offering static typechecking and more granular control over compilation. Unlike most statically-typed languages, \Ace's type system can be extended from within. The extension mechanism guarantees that extensions cannot interfere with one another by organizing extension logic around types. We show that such \emph{active types} can express the static and dynamic semantics of several primitive functional, object-oriented and parallel programming abstractions, as well as domain-specific abstractions, all from within libraries. 
