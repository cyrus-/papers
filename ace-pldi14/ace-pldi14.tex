...
Generic functions can be generated directly from ASTs as well, providing Ace with support for  straightforward metaprogramming. Listing \ref{metaprogramming} shows how to generate two more generic functions, \verb|scale| and \verb|double|. The latter is derived from the former using a library for manipulating Python syntax trees, \verb|astx|. In particular, its \verb|specialize| function replaces uses of the second argument of \verb|scale| with the literal \verb|2| (and changes the function's name), leaving a function of one argument.
