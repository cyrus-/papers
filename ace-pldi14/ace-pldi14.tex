...
\maketitle
\begin{abstract}
Evidence suggests that programmers are reluctant to adopt new programming languages to gain access to new abstractions, even when they agree that these abstractions could be valuable for their work. This suggests a need for languages designed to satisfy two related design criteria: \emph{compatibility} with existing languages, tools and ecosystems; and \emph{extensibility}, so that adopting new abstractions does not require adopting a new language.

In this paper, we introduce \Ace, a language compatible with the tools and infrastructure developed for Python, among the most  widely-adopted dynamically-typed language today. While Python and other similar languages are considered satisfactory for small-scale scripting tasks, \Ace~is designed for situations where correctness and performance are particularly important, offering a  static type system and more granular control over compilation. Unlike most statically-typed languages, however, the \Ace~type system can be extended from within. The extension mechanism, called \emph{active typechecking and translation}, guarantees that extensions cannot interfere  with one another by restricting the scope of extension logic, which must be associated with a type, around expressions of that type. We show that this mechanism remains powerful enough to express the static and dynamic semantics of several common functional, object-oriented and parallel programming primitives, as well as varied domain-specific abstractions, all from within libraries. 
