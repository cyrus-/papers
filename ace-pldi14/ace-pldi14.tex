...
Generic functions can be generated directly from ASTs as well, providing Ace with support for  straightforward metaprogramming. Listing \ref{metaprogramming} shows how to generate two more generic functions, \verb|thresh_scale| and \verb|thresh_dbl|. The latter is derived from the former using a library for manipulating Python syntax trees, \verb|astx|. In particular, its \verb|specialize| function replaces uses of the second argument of \verb|thresh_scale| with the literal \verb|2.0| (and changes the function's name), leaving a function of one argument.
 
\subsection{Types}
To go from a generic function to a \emph{concrete function}, we must provide a type assignment for its arguments. We will show different ways to do this in Secs. \ref{procedural} and \ref{implicit}. Before we can do this, however, we must understand what types are.
...
Listing 2 shows three types, on lines 7-9, being assigned to variables for convenience (a construct similar to \verb|typedef| in C-like languages or \verb|type| in Haskell). These types all come from the \verb|clx| library imported on line 1, none are built into Ace itself.
...
\end{lstlisting}
\caption{Compiling \texttt{listing\ref{mapadd5dbl}.py} using the \texttt{acec} compiler.}
\label{mapc}
\end{codelisting}
\begin{codelisting}
