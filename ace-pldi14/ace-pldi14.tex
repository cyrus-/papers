...
Functions containing run-time behavior, like \verb|map|, are governed by a semantics that can differ from Python's (in ways that we will describe below), but they share Python's syntax. As a consequence, users of Ace  immediately benefit from an ecosystem of well-developed tools that work with Python code, including parsers, code highlighters, editor modes, style checkers and documentation generators. 

\subsection{OpenCL as an Active Library}
The code in this section uses \verb|clx|, an example library implementing the semantics of the OpenCL programming language and extending it with some additional useful types, which we will discuss shortly. Ace itself provides no built-in support for OpenCL.

To briefly review, in OpenCL, developers can define functions, called {\em kernels}, that execute on \emph{devices} like GPUs or multi-core CPUs \cite{opencl11}. Code on the host CPU launches a kernel over tens of thousands of threads. Each thread executes the same kernel but has access to a unique index, called its \emph{global id} (a data-parallel SPMD programming model). Kernel code is written in a variant of C99 extended with some new primitive types and operators, which we will introduce as needed in our examples below.
...
\label{map}
\end{codelisting}
Lines 3 and 4 introduce the definition of  \verb|map|, an Ace function taking three arguments, \verb|input|, \verb|output| and \verb|f|, governed by the active base \verb|clx.std_base|. The body of this function, highlighted in grey for emphasis, does not have Python's semantics. Instead, it will be governed by the active base as well as logic associated with the types of the arguments and expressions it contains. 

Generic functions in Ace are instances of \verb|ace.GenericFn|. Generic functions consist of an abstract syntax tree, an active base and an active target\todo{call it this in intro}. The job of the \emph{decorator} on line 3 is to replace the Python function on lines 4-8 with an Ace generic function having the same syntax tree and the specified active base and active target. Because no active target was provided, the active base  provides a default target in this case. The default target provided by \verb|clx.std_base| is \verb|clx.opencl|, but, as we will discuss in Sec. \ref{bases}, other bases, like \verb|clx.cuda|, a vendor-specific language similar to OpenCL, as well as Python itself (\verb|ace.python|), can also be defined in libraries. One base can support multiple targets.

Python decorators are simply syntactic sugar for applying a function (here, the result of \verb|ace.fn(clx.std_base)|) directly to the function definition being decorated \cite{python-decorators}. In other words, line 3 could be removed and replaced by inserting the equivalent statement \verb|map = ace.fn(clx.std_base)(map)| to line 9. 
 The decorator applied here extracts the abstract syntax tree from \verb|map| using the Python standard  library packages  \verb|inspect| (to retrieve source code) and \verb|ast| (to parse it into a syntax tree). 
 
\subsection{Concrete Functions}
The definition of \verb|map| in Listing \ref{map} could be meaningful for many different type assignments to \verb|input|, \verb|output| and \verb|f|. The body of the function imposes certain constraints on these types, but there is not necessarily a unique type assignment that satisfies them. In a sense, then, \verb|map| is a \emph{family} of functions. Any type assignment for which the operations in the function's body are well-defined can be generated from \verb|map|. We call an Ace function for which types have been assigned a \emph{concrete function}. Concrete functions are instances of \verb|ace.ConcreteFn|. We will show different ways types can be assigned to the arguments of a generic function, and how this, in tandem with the active base, controls the semantics and compilation of the function body in Secs. \ref{procedural} and \ref{implicit}. First, we must understand how types are defined and used in Ace.

\subsection{Types}
In Ace, types are themselves values that can be manipulated at compile-time (unlike most contemporary languages, where user-defined types are simply declared at compile-time and cannot be defined or manipulated programmatically). In particular, types are instances of a Python class that implements the \verb|ace.ActiveType| interface. Python, then, serves also as the \emph{type-level language} of Ace (see related work in Sec. \ref{related}). 

Listing 2 shows three types, on lines 7-9, being assigned to variables for convenience (this is similar to \verb|typedef| in C-like languages or \verb|type| in Haskell). These types all come from the \verb|clx| library imported on line 1, none are built into Ace itself.

The type named \verb|F| on line 7 corresponds to a type in OpenCL, \verb|global float*|: a pointer to a 32-bit floating point number stored in the compute device's global memory (one of four memory spaces defined by OpenCL, see \cite{opencl11}). Here, \verb|clx.float| is an Ace type corresponding to \verb|float| in OpenCL, and \verb|clx.Ptr| is an Ace type family corresponding to pointer types, indexed by both an address space (here, \verb|clx.global_|) and the type being pointed to. The \verb|clx| library contains a complete implementation of the OpenCL type system, which is based on C99. We will return to how this works in Section \ref{att}. We note that we are being more verbose than necessary for pedagogical reasons. The \verb|clx| library defines more concise naming schemas for OpenCL's built-in types: \verb|F| could be written \verb|clx.gp(clx.f32)|. We also note that Ace is \emph{unopinionated} about issues like memory safety. We will show how to implement safer alternatives to raw pointers in Sec. \ref{examples}, but we allow users to choose a low level of abstraction when they wish.(see Sec. \ref{discussion}).

Listing \ref{mapadd5dbl} shows how to assign types to the arguments of \verb|map| so that this logic can be typechecked and compiled. 
...
The \verb|map| function defined in Listing \ref{map} will compile to one or more kernels that applies a transfer function, \verb|f|, to the element of the input array, \verb|input|, corresponding to its global id. It writes the result of this call into the corresponding location in the provided output array, \verb|output|. If the kernel is running in thread 0, it prints a message. The functions \verb|get_global_id| and \verb|printf| are OpenCL primitive functions. To emphasize again that the semantics of this code differs from Python's, we note that these functions have no meaning in Python.
...
This example demonstrates several key design decisions that characterize Ace: static typing of run-time behavior, a Python-based syntax, a phase distinction between compile-time and run-time logic, and programmatic 
\subsection{Programmatic Compilation}
\begin{codelisting}
\lstinputlisting[linebackgroundcolor={\btLstHL{5-5}}]{listing4.py}
\caption{[\texttt{listing\ref{mapadd5dbl}.py}] The generic \texttt{map} function compiled to map the \texttt{add\_5} function over two  types of input.}
\label{mapadd5dbl}
\end{codelisting}
...
\end{lstlisting}
\caption{Compiling \texttt{listing2.py} using the \texttt{acec} compiler.}
\label{mapc}
\end{codelisting}
\begin{codelisting}
\lstinputlisting[style=OpenCL]{listing5.cl}
\caption{[\texttt{listing\ref{mapadd5dbl}.cl}] The OpenCL file generated by Listing \ref{mapc}.}
\label{mapout}
\end{codelisting}
...
\caption{[\texttt{listing\ref{metaprogramming}.py}] Metaprogramming with Ace, showing how to construct generic functions from both strings and abstract syntax trees, and how to manipulate syntax trees at compile-time.}
...
\subsection{Implicit Compilation and Execution}\label{compenv}\label{backend}
