...
Being Python values, types can be assigned to variables for convenience (a construct similar to \verb|typedef| in C or \verb|type| in Haskell). Types, like all compile-time mechanisms in Ace (functions, bases, etc.), do not have mutable state and operate in a referentially transparent manner.% These types are all implemented in the \verb|clx| library imported on line 1, none are built into Ace itself.

The type named \verb|T1| on line \ref{compscript}.7 corresponds to the OpenCL type \verb|global float*|: a pointer to a 32-bit floating point number stored in the compute device's global memory (one of four memory spaces defined by OpenCL \cite{opencl11}). It is constructed by applying \verb|clx.Ptr|, which is an Ace type constructor corresponding to pointer types, to an address space (here, \verb|clx.global_|) and the type being pointed to, \verb|clx.float|, the Ace type corresponding to \verb|float| in OpenCL. 
