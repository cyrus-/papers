...
In a compiler for a monolithic language, there would be a \emph{syntax-directed} protocol governing typechecking and translation. For example, in a functional language, one would declare datatypes that captured all forms of types and expressions, and the typechecker would perform exhaustive case analysis over the expression forms, implementing the logic described above in the cases for subscript (\verb|e1[e2]|) and addition (\verb|e1 + e2|). The visitor pattern typically used in object-oriented languages implements essentially the same protocol. This does not work for an extensible language because new cases cannot be added to the types and functions (in a modular, safely composable manner).

Instead of taking a syntax-directed approach, the Ace compiler's typechecking and translation phases take a \emph{type-directed approach}. When encountering a compound expression (e.g. \verb|e1[e2]|), Ace defers control over typechecking and translation to the active type of a designated subexpression. The choice of subexpression is determined by Ace's fixed \emph{dispatch protocol}, which is designed taking into consideration Python's semantics for the same form. Below are examples of these choices. Due to space constraints, we do not show the full protocol.
\begin{itemize}
...
When typechecking a compound expression or statement, the Ace compiler calls a method of the form \verb|type_|$X$ on the active type that has responsibility over it according to the dispatch protocol, where $X$ is typically the name of the syntactic form, taken from the Python grammar \cite{pythonast}. For example, the typing logic for the \verb|Subscript| and \verb|BinOp| forms when operating on OpenCL pointers are shown beginning on lines \ref{pointers}.8 and \ref{pointers}.20, respectively.  The typing methods are provided a \verb|context| and the abstract syntax tree of the \verb|node| being considered. The context is an instance of \verb|ace.Context| and tracks information like variable types, as coordinated by the base. It also includes a method \verb|type| that can be used to assign  type to subexpressions as needed. For example, line \ref{pointers}.8 checks if the type of the slice is an integer type. \verb|Integer| is simply another class in the \verb|clx| library that implements the \verb|ace.ActiveType| interface. Specific integer types like \verb|int| and \verb|ptrdiff_t| are instances of this class (details not shown). Note that the subexpressions are not \emph{automatically} assigned types. This is useful because it allows types to contextually reinterpret them to support more complex constructs (e.g. pattern matching on datatypes, see Sec. \ref{datatypes}).

If a type error is detected, \verb|ace.TypeError| can be raised with an appropriate error message and a reference to the node that produced it (so that Ace can extract line and column numbers). We note that error messages are an important and sometimes overlooked facet of {ease-of-use} \cite{marceau2011measuring}. A common frustration with general-purpose abstraction mechanisms is that they can produce  verbose and cryptic error messages revealing implementation details of the abstraction being encoded (e.g. template metaprogramming \cite{template-metaprogramming}). 
